...
const combatScrollByItemHrid = Object.freeze(Object.fromEntries(
    combatScrollOptions.map((option) => [option.itemHrid, option])
));
...
export function normalizeCombatScrolls(raw, options = undefined) {
    ...
    const definitions = options == null ? combatScrollByItemHrid : Object.fromEntries(
        getCombatScrollOptions(options).map((option) => [option.itemHrid, option])
    );
    const normalized = {};
