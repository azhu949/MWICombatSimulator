...
describe('gameDataIndex', () => {
  ...
  it('keeps the generated combat scroll item index in sync with its pipeline input', () => {
    ...
    const expected = Object.fromEntries(
      Object.values(itemDetailSource)
        .filter((item) => item?.hrid && item?.categoryHrid === categoryHrid && item?.scrollDetail?.personalBuffTypeHrid)
        .map((item) => [
          item.hrid,
          {
            hrid: item.hrid,
  ...
  it('exposes a compact read-only enhancement data index', () => {
    ...
    const enchantedGloves = enhancementData.enhanceableItems.find((item) => item.hrid === '/items/enchanted_gloves');
    ...
    expect(
      enhancementData.materialItems.some((item) => item.hrid === enhancementData.specialItemHrids.philosophersMirror),
    ).toBe(true);
    expect(
      enhancementData.protectionItems.some((item) => item.hrid === enhancementData.specialItemHrids.mirrorOfProtection),
    ).toBe(true);
  });
  ...
  it('includes data-driven enhancement equipment, drinks, and external buffs', () => {
    ...
    expect(enhancementData.supportEquipment.some((item) => item.hrid === '/items/philosophers_necklace')).toBe(true);
    expect(enhancementData.supportEquipment.find((item) => item.hrid === '/items/guzzling_pouch')).toMatchObject({
    ...
    expect(enhancementData.enhancingDrinks.find((item) => item.hrid === '/items/enhancing_tea')).toMatchObject({
      durationSeconds: 300,
    });
  ...
  it('exposes compact gathering and production actions, equipment, and drinks', () => {
    ...
    expect(
      skillingData.actions.find((action) => action.hrid === '/actions/tailoring/artificer_cape_refined'),
    ).toMatchObject({
