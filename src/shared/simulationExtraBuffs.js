...
function buildBuffFromTemplate(template, level = 1) {
  ...
  const ratioBoost =
    toFiniteNumber(template.ratioBoost, 0) + toFiniteNumber(template.ratioBoostLevelBonus, 0) * (normalizedLevel - 1);
  const flatBoost =
    toFiniteNumber(template.flatBoost, 0) + toFiniteNumber(template.flatBoostLevelBonus, 0) * (normalizedLevel - 1);
  ...
  return {
    ...
    flatBoost,
    flatBoostLevelBonus: 0,
    startTime: typeof template.startTime === 'string' && template.startTime ? template.startTime : DEFAULT_START_TIME,
