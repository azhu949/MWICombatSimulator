    ...
    const slots = rawSlots
      .map((slot, index) => {
        ...
        return {
          ...
          itemHrid,
          isActive: slot?.isActive === true ? true : slot?.isActive === false ? false : null,
        };
      })
      .filter(Boolean);
...
export function normalizeSkillingStoredProfile(rawProfile) {
  ...
  for (const [skillHrid, rawSkill] of Object.entries(isPlainObject(rawProfile.skills) ? rawProfile.skills : {})) {
    ...
    const maximumLevel = hrid === '/skills/total_level' ? Number.MAX_SAFE_INTEGER : SKILLING_MAX_LEVEL;
    skills[hrid] = {
      level: clamp(integer(rawSkill.level, 1), 1, maximumLevel),
      experience: Number.isFinite(Number(rawSkill.experience)) ? Math.max(0, Number(rawSkill.experience)) : null,
    };
  }
  ...
  return {
    ...
    inventory,
    equipment,
    buffsBySource: cloneJsonValue(isPlainObject(rawProfile.buffsBySource) ? rawProfile.buffsBySource : {}, {}),
...
export const useSkillingStore = defineStore('skilling', () => {
  ...
  const balancedCostTolerance = ref(
    normalizeSkillingBalancedCostTolerance(persisted.balancedCostTolerance ?? SKILLING_BALANCED_COST_TOLERANCE),
  );
  const runScope = ref(SKILLING_RUN_SCOPE_ALL);
  const selectedRunSkillHrid = ref(
    String((skillingData?.skillHrids || []).find((skillHrid) => typeof skillHrid === 'string' && skillHrid) || ''),
  );
  ...
  const snapshotAgeMs = computed(() =>
    profile.value?.importedAt ? Math.max(0, Date.now() - finiteNumber(profile.value.importedAt, Date.now())) : null,
  );
  const expiredBuffCount = computed(() =>
    Math.max(0, ...Object.values(plansBySkill.value).map((plan) => integer(plan?.expiredBuffCount, 0))),
  );
  ...
  watch(
    [() => simulator.pricing?.lastFetchedAt, () => simulator.pricing?.sourceUrl, () => simulator.pricing?.overrides],
    () => {
      invalidateResult();
    },
    { deep: true, flush: 'sync' },
  ...
  function setRunScope(scope) {
    const normalized = scope === SKILLING_RUN_SCOPE_SINGLE ? SKILLING_RUN_SCOPE_SINGLE : SKILLING_RUN_SCOPE_ALL;
  ...
  async function run() {
    ...
    const selectedSkillHrid = skillHrids.value.includes(selectedRunSkillHrid.value)
      ...
      : skillHrids.value[0];
    const runSkillHrids =
      runScope.value === SKILLING_RUN_SCOPE_SINGLE ? [selectedSkillHrid].filter(Boolean) : [...skillHrids.value];
