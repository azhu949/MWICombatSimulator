...
import { enhancementData, itemDetailIndex } from '../shared/gameDataIndex.js';
import {
...
export const LEGACY_ENHANCEMENT_STORAGE_KEYS = Object.freeze(['mwi.enhancement.v1', 'mwi.enhancement']);
...
function normalizePriceOverrides(rawOverrides) {
  ...
  for (const [rawKey, rawValue] of Object.entries(isPlainObject(rawOverrides) ? rawOverrides : {})) {
    ...
    const quote = {};
    for (const [sourceSide, normalizedSide] of [
      ['ask', 'ask'],
      ['bid', 'bid'],
      ['a', 'ask'],
      ['b', 'bid'],
...
export function normalizeEnhancementPersistedState(rawState = {}) {
  ...
  const config = {
    ...
    protectionItemHrid: validProtectionItemHrids.has(requestedProtectionItemHrid) ? requestedProtectionItemHrid : '',
    laborRatePerHour: nonNegativeNumber(rawConfig.laborRatePerHour ?? rawConfig.hourlyRate, defaults.laborRatePerHour),
    markupRate: clamp(
      rawConfig.markupRate == null
        ? nonNegativeNumber(rawConfig.markupPercent ?? rawConfig.percentRate ?? rawConfig.percent_rate, 0) / 100
        : nonNegativeNumber(rawConfig.markupRate, defaults.markupRate),
      0,
      10,
    ),
...
export function persistEnhancementState(state, storage = globalThis?.localStorage) {
  ...
  const persistedConfig = Object.fromEntries(
    Object.entries(normalized.config).filter(([key]) => !ENHANCEMENT_SESSION_CONFIG_KEYS.has(key)),
  );
  try {
    target.setItem(
...
function effectiveAnalysisConfig(config) {
  return {
    ...config,
    protectionItemHrid: config.protectionMode === 'manual' ? normalizeHrid(config.protectionItemHrid) : '',
  };
}
...
function sortableStrategyCost(strategy) {
  ...
  return incremental != null && Number.isFinite(Number(incremental)) ? Number(incremental) : Infinity;
}

function adaptStrategy(strategy, recommendedProtectAt) {
  const materialCost =
    strategy.materialPricesAvailable === false ? null : strategy.expectedActions * strategy.materialCostPerAction;
...
export const useEnhancementStore = defineStore('enhancement', () => {
  ...
  const itemOptions = computed(() =>
    (enhancementData?.enhanceableItems || [])
      ...
      .sort(
        (left, right) =>
          finiteNumber(left?.sortIndex, Infinity) - finiteNumber(right?.sortIndex, Infinity) ||
  ...
  const teaOptions = computed(() =>
    (enhancementData?.enhancingDrinks || []).filter(
      (drink) =>
  ...
  const supportSlotOptions = computed(() =>
    (enhancementData?.supportEquipment || []).map((item) => ({
      ...item,
      slot:
        String(item.equipmentType || '')
          .split('/')
  ...
  const equipmentOptionsBySlot = computed(() =>
    Object.fromEntries(
      SUPPORT_SLOT_KEYS.map((slot) => [slot, supportSlotOptions.value.filter((item) => item.slot === slot)]),
    ),
  );
  ...
  const rawAnalysis = computed(() => {
    ...
    return analyzeEnhancementStrategies(effectiveAnalysisConfig(config), enhancementData, pricing.value);
  });
  const strategyRows = computed(() => {
    ...
    return source.strategies
      ...
      .sort(
        (left, right) =>
          sortableStrategyCost(left) - sortableStrategyCost(right) || (left.protectAt ?? -1) - (right.protectAt ?? -1),
      );
  });
  const recommendedStrategy = computed(
    () => strategyRows.value.find((strategy) => strategy.recommended) || strategyRows.value[0] || null,
  );
  ...
  const protectionOptions = computed(() =>
    (rawAnalysis.value?.protectionCandidates || []).map((candidate) => ({
  ...
  const startingItemPrice = computed(() => {
    const resolved =
      recommendedStrategy.value?.startingItem ||
      (config.itemHrid
        ? resolveStartingItemPrice(effectiveAnalysisConfig(config), enhancementData, pricing.value)
        : null);
    return {
      ...(resolved || {}),
  ...
  const materialRows = computed(() => {
    ...
    const rows = new Map();
    const addRow = (hrid, { count = 0, quantity = 0, mode = 'ask', kind = 'material', resolvedPrice = null } = {}) => {
      if (!hrid) {
        return;
      }
      ...
      const resolved =
        resolvedPrice ||
        resolveEnhancementPrice(pricing.value, hrid, {
          mode,
    ...
    for (const protection of protectionOptions.value) {
      addRow(protection.hrid, {
        quantity:
          recommendedStrategy.value?.protectionItemHrid === protection.hrid
  ...
  const mirrorPlan = computed(() => {
    ...
    const buildDirectPlans = (itemHrid, maximumLevel, useConfiguredProtection) => {
      ...
      const analysisConfig = {
        ...
        startingItemPriceOverride:
          itemHrid === config.itemHrid && integer(config.startLevel, 0) === 0
            ? effectiveConfig.startingItemPriceOverride
            : null,
        protectionItemHrid: useConfiguredProtection ? effectiveConfig.protectionItemHrid : '',
      };
      const directPlans = [];
    ...
    const directCost = Number(
      targetPlans.directPlans[targetLevel]?.totalInvestment ?? targetPlans.directPlans[targetLevel]?.cost,
    );
    const materials = plan.requirements.map((requirement) =>
      requirement.type === 'mirror'
        ? {
            ...requirement,
            hrid: mirrorHrid,
  ...
  const decompositionValue = computed(() =>
    config.itemHrid
      ? calculateDecompositionValue(
          {
            itemHrid: config.itemHrid,
  ...
  watch(
    [() => config.startLevel, () => config.targetLevel],
    ([startValue, targetValue], [previousStartValue] = []) => {
  ...
  watch(
    ...
    (rows) => {
      ...
      const available = rows.some((strategy) => strategy.id === requested || strategy.strategyId === requested);
      if (!available) {
  ...
  function resolveRiskStrategy() {
    ...
    return (
      strategyRows.value.find((strategy) => strategy.id === requested || strategy.strategyId === requested) ||
  ...
  async function runRisk() {
    ...
    const sampleCount = clamp(integer(config.sampleCount, DEFAULT_MONTE_CARLO_SAMPLES), 1024, 1_000_000);
