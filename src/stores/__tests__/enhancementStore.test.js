...
describe('enhancementStore', () => {
  ...
  it('discovers future enhancement support equipment slots', () => {
    expect(deriveEnhancementSupportSlotKeys([{ equipmentType: '/equipment_types/future_slot' }])).toContain(
      'future_slot',
    );
  });
  ...
  it('filters enhanceable items by equipment type instead of the shared item category', () => {
    ...
    expect(new Set(store.itemOptions.map((item) => item.categoryHrid))).toEqual(
      new Set(['/item_categories/equipment']),
    );
  ...
  it('shows acquisition estimates for non-tradable protection without auto-selecting them', async () => {
    ...
    expect(store.materialRows.find((row) => row.kind === 'protection' && row.hrid === cloakHrid)).toMatchObject({
      source: 'acquisition_estimate',
      expectedContainers: 25,
      missing: false,
    });
  ...
  it('builds strategy, mirror, and decomposition views from current data', () => {
    ...
    expect(store.strategyRows.map((row) => row.id)).toEqual(expect.arrayContaining(['none', 'protect-2']));
    expect(store.strategyRows[0].totalInvestment ?? store.strategyRows[0].incrementalCost).toBeLessThanOrEqual(
      store.strategyRows.at(-1).totalInvestment ?? store.strategyRows.at(-1).incrementalCost,
    );
    ...
    expect(store.materialRows).toEqual(
  ...
  it('uses base equipment and mirror-action labor for refined-item mirror plans', () => {
    ...
    expect(store.mirrorPlan.materials).toEqual(
      expect.arrayContaining([
  ...
  it('uses a current +0 starting-item override in the independent mirror plan', () => {
    const store = useEnhancementStore();
    const refinedItem = enhancementData.enhanceableItems.find(
      (item) => item.isTradable === false && item.baseItemHrids?.length > 0,
    );
    const baseItemHrid = refinedItem.baseItemHrids[0];
    const mirrorHrid = enhancementData.specialItemHrids.philosophersMirror;
  ...
  it('keeps overlapping ask/bid and material/protection price rows distinct', () => {
    ...
    expect(store.materialRows.filter((row) => row.hrid === essenceHrid)).toEqual(
      expect.arrayContaining([
    ...
    expect(store.materialRows.find((row) => row.hrid === essenceHrid && row.priceMode === 'ask')).toMatchObject({
      unitPrice: 100,
      source: 'ask',
      overrideValue: null,
    });
    expect(store.materialRows.find((row) => row.hrid === essenceHrid && row.priceMode === 'bid')).toMatchObject({
      unitPrice: 79,
      source: 'override',
      overrideValue: 79,
    });
    ...
    expect(store.materialRows.filter((row) => row.hrid === '/items/trainee_enhancing_charm')).toEqual(
      expect.arrayContaining([
  ...
  it('lists enabled tea costs with their expected quantities', () => {
    ...
    expect(consumables.map((row) => row.hrid)).toEqual(
      expect.arrayContaining(['/items/enhancing_tea', '/items/blessed_tea', '/items/wisdom_tea']),
    );
    ...
    store.config.targetLevel = 2;
    expect(store.mirrorPlan.mirrorActionCost).toBeCloseTo(store.recommendedStrategy.consumableCostPerAction, 10);
  });
