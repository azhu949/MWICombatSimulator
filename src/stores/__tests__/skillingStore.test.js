...
describe('skillingStore', () => {
  ...
  it('defaults old or invalid persisted optimization modes to cost', () => {
    ...
    expect(normalizeSkillingPersistedState({ optimizationMode: 'unknown' }).optimizationMode).toBe(
      SKILLING_OPTIMIZATION_MODE_COST,
    );
    expect(
      normalizeSkillingPersistedState({ optimizationMode: SKILLING_OPTIMIZATION_MODE_BALANCED }).optimizationMode,
    ).toBe(SKILLING_OPTIMIZATION_MODE_BALANCED);
    expect(
      normalizeSkillingPersistedState({ optimizationMode: SKILLING_OPTIMIZATION_MODE_SPEED }).optimizationMode,
    ).toBe(SKILLING_OPTIMIZATION_MODE_SPEED);
  });

  it('defaults and clamps the persisted balanced cost tolerance', () => {
    expect(normalizeSkillingPersistedState({}).balancedCostTolerance).toBe(SKILLING_BALANCED_COST_TOLERANCE);
    expect(normalizeSkillingPersistedState({ balancedCostTolerance: 'invalid' }).balancedCostTolerance).toBe(
      SKILLING_BALANCED_COST_TOLERANCE,
    );
