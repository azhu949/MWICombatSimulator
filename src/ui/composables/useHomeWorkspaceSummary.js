...
export function useHomeWorkspaceSummary(combatPreview) {
    ...
    const currentRunScopeLabel = computed(() => {
        const scope = simulator.availableRunScopes.find(
            (entry) => entry.value === simulator.simulationSettings.runScope,
        );
        return t(
    ...
    const summaryMetricRows = computed(() => {
        ...
        return [
            metric(
                t("common:vue.results.xpPerHour", "XP/h"),
                row ? formatNumber(row.totalXpPerHour, 0) : "-",
                "accent",
            ),
            metric(
