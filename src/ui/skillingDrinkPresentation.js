...
export function buildSkillingDrinkStatuses(segment) {
  ...
  return (Array.isArray(segment?.drinks) ? segment.drinks : [])
    .map((drink) => {
      ...
      const finalStateKnown = hasEndingItems && Object.prototype.hasOwnProperty.call(endingItems, itemHrid);
      const phaseLifecycleKnown = phases.some(
        (phase) =>
          (Array.isArray(phase?.drinks) &&
            phase.drinks.some((item) => String(item?.itemHrid || '').trim() === itemHrid)) ||
          Object.prototype.hasOwnProperty.call(phase?.endingDrinkState?.itemsByHrid || {}, itemHrid),
      );
      const endingKnown = finalStateKnown || phaseLifecycleKnown;
      const remainingSeconds = finalStateKnown ? nonnegativeNumber(endingItems[itemHrid]?.remainingSeconds) : 0;
      return {
        itemHrid,
        consumedCount,
        ...
        remainingSeconds,
        usedUp:
          endingKnown && remainingSeconds <= EPSILON && (consumedCount > EPSILON || carriedDurationSeconds > EPSILON),
      };
    })
    .filter(Boolean);
}
