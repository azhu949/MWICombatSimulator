  ...
  <section class="mt-2 border-t border-border" :data-trigger-target="targetId">
    ...
    >
      <span class="min-w-0 flex-1">
        <span class="flex flex-wrap items-center gap-1.5">
          <span class="text-xs font-semibold text-foreground">{{ t("common:vue.home.trigger.summaryTitle", "Trigger conditions") }}</span>
      ...
      <span class="flex shrink-0 items-center gap-1 text-xs font-medium text-primary">
        ...
        <ChevronDown class="size-4 transition-transform duration-200 motion-reduce:transition-none" :class="expanded ? 'rotate-180' : ''" />
      </span>
    </button>
    ...
    <div v-if="expanded" class="space-y-3 border-t border-border px-1 pt-3" data-trigger-editor>
      <div class="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
        <div class="min-w-0">
          ...
          <p class="text-xs text-muted-foreground">
            {{ draft.length === 0
              ? t("common:vue.home.trigger.noRulesHint", "No conditions: activate as soon as cooldown and resource requirements allow.")
      ...
      <div class="divide-y divide-border">
        <div
          v-for="(trigger, rowIndex) in draft"
          :key="`trigger-${rowIndex}`"
        ...
        >
          <div class="mb-2 flex items-center justify-between gap-2">
            ...
            <button type="button" class="text-xs font-medium text-destructive hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" @click="removeRule(rowIndex)">
          ...
          <div class="grid grid-cols-1 gap-2 lg:grid-cols-2">
            ...
            <label class="block">
              ...
              >
                ...
                <SelectContent>
                  <SelectItem :value="EMPTY_SELECT_VALUE">{{ t("common:vue.common.select", "Select") }}</SelectItem>
                  <SelectItem v-for="comparator in comparatorOptions(trigger.conditionHrid)" :key="comparator.hrid" :value="comparator.hrid">
...
const draftValid = computed(() => props.draft.every((entry) => (
...
const ruleCountLabel = computed(() => t(
  "common:vue.home.trigger.ruleCount",
  "{{count}} conditions",
  { count: props.currentRules.length },
));
