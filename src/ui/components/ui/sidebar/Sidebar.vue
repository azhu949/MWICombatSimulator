<template>
  ...
  <DialogRoot :open="mobileOpen" @update:open="setMobileOpen">
    <DialogPortal>
      <DialogOverlay
        class="fixed inset-0 z-50 bg-modal-backdrop data-[state=closed]:animate-out data-[state=open]:animate-in data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 md:hidden"
      />
      <DialogContent
      ...
      >
        ...
        <DialogDescription id="mobile-sidebar-description" class="sr-only">{{
          t('common:vue.app.applicationNavigation', 'Application navigation')
        }}</DialogDescription>
