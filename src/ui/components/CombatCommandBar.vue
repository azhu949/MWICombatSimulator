    ...
    <div class="mx-auto flex max-w-[1500px] flex-col gap-2.5">
      <div class="flex min-w-0 items-center gap-2">
        <div class="hidden shrink-0 items-center gap-2 2xl:flex">
          ...
          <Button type="button" variant="outline" size="sm" :disabled="queueActionsDisabled || !hasBaseline || partyMismatch" @click="emit('add-queue')">
            <ListPlus />{{ t("common:queue.addToQueue", "Add To Queue") }}
          </Button>
          <Button type="button" size="sm" :disabled="queueActionsDisabled || !hasBaseline || itemCount === 0 || partyMismatch" @click="emit('run-queue')">
            <Play />{{ t("common:queue.runQueue", "Run Queue") }}
          </Button>
          <Button type="button" variant="destructive" size="sm" :disabled="queueActionsDisabled || itemCount === 0" @click="emit('clear-queue')">
            <Trash2 />{{ t("common:queue.clearQueue", "Clear Queue") }}
          </Button>
        </div>

        <Button class="2xl:hidden" type="button" size="sm" :disabled="queueActionsDisabled || !hasBaseline || itemCount === 0 || partyMismatch" @click="emit('run-queue')">
          <Play />{{ t("common:queue.runQueue", "Run Queue") }}
        </Button>
        <DropdownMenuRoot>
          <DropdownMenuTrigger as-child>
            <Button class="2xl:hidden" type="button" variant="outline" size="icon-sm" :aria-label="t('common:vue.app.moreActions', 'More actions')">
              <Ellipsis />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuPortal>
            <DropdownMenuContent class="z-50 min-w-48 rounded-md border border-border bg-popover p-1 text-popover-foreground shadow-xl" :side-offset="6" align="start">
              <DropdownMenuItem class="command-menu-item" :disabled="queueActionsDisabled" @select="emit('set-baseline')"><Gauge />{{ t("common:queue.setBaseline", "Set Baseline") }}</DropdownMenuItem>
              <DropdownMenuItem class="command-menu-item" :disabled="queueActionsDisabled || !hasBaseline || partyMismatch" @select="emit('add-queue')"><ListPlus />{{ t("common:queue.addToQueue", "Add To Queue") }}</DropdownMenuItem>
              <DropdownMenuItem class="command-menu-item text-destructive" :disabled="queueActionsDisabled || itemCount === 0" @select="emit('clear-queue')"><Trash2 />{{ t("common:queue.clearQueue", "Clear Queue") }}</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenuPortal>
        </DropdownMenuRoot>
        ...
        <div class="flex min-w-0 flex-1 items-center gap-1.5 overflow-x-auto pl-1 [scrollbar-width:thin]">
          ...
          >
            <input v-model="player.name" class="w-[72px] bg-transparent text-xs font-semibold outline-none" :aria-label="t('common:player', 'Player')" @click.stop @focus="emit('select-player', player.id)" />
            <input v-model="player.selected" type="checkbox" class="size-3.5 accent-primary" :aria-label="t('common:vue.app.simToggle', 'Sim')" @click.stop />
          </div>
        </div>

        <div class="hidden shrink-0 items-center gap-3 text-xs text-muted-foreground xl:flex">
          <span>{{ t("common:queue.queueList", "Queue List") }} <strong class="text-foreground">{{ itemCount }}</strong></span>
          <span>{{ t("common:vue.queue.queueProgress", "Queue Progress") }} <strong class="text-foreground">{{ queueProgressText }}</strong></span>
        </div>
      </div>
      ...
      <div v-if="showRuntimeSummary" class="flex items-center gap-3 border-t border-border pt-2">
        <span class="shrink-0 text-[10px] font-semibold uppercase text-muted-foreground">{{ t("common:vue.app.runtime", "Runtime") }}</span>
        ...
        <Button v-if="runtimeError" type="button" variant="ghost" size="sm" class="text-destructive" @click="emit('view-error', runtimeError)">
