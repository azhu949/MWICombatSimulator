const INPUT_SUM_FIELDS = Object.freeze(['count', 'ownedCount', 'purchaseCount', 'opportunityCost', 'purchaseCost']);
...
function equipmentStrategiesForSegment(segment) {
  if (Array.isArray(segment?.equipmentStrategies)) {
    return segment.equipmentStrategies.map((strategy) => ({
      ...strategy,
      equipment: (Array.isArray(strategy?.equipment) ? strategy.equipment : []).map((item) => ({ ...item })),
    }));
  }
  return [
    {
      equipmentSignature: equipmentSignature(segment),
      equipment: (Array.isArray(segment?.equipment) ? segment.equipment : []).map((item) => ({ ...item })),
...
export function buildSkillingRangeSummary(segments, options = {}) {
  ...
  return {
    ...
    costPerExperience: totals.gainedExperience > 0 ? totals.netCost / totals.gainedExperience : 0,
    materialPurchaseCostPerExperience:
      totals.gainedExperience > 0 ? totals.materialPurchaseCost / totals.gainedExperience : 0,
    experiencePerHour: totals.durationHours > 0 ? totals.gainedExperience / totals.durationHours : 0,
    inputItems: recalculateUnitPrices(mergeRows(sourceSegments, 'inputItems', INPUT_SUM_FIELDS), [
      {
    ...
    outputItems: recalculateUnitPrices(mergeRows(sourceSegments, 'outputItems', OUTPUT_SUM_FIELDS), [
      {
