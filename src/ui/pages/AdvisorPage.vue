      ...
      <div class="surface-panel overflow-hidden">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div class="space-y-2">
            ...
            <DisclosurePanel
              :title="t('common:advisor.scoreExplainTitle', '评分说明：综合分与置信度怎么算')"
              class="max-w-3xl"
            >
              <div class="space-y-3 text-xs leading-6 text-foreground/85">
                <div>
                  <p class="font-semibold text-foreground">
                    {{ t('common:advisor.scoreExplainCompositeHeading', '综合分怎么算？') }}
                  </p>
                    ...
                    <li>
                      {{
                        t(
                          ...
                          '按 4 个维度评估每个目标：每日收益、每小时经验、每小时击杀、每小时死亡（安全性）。',
                        )
                      }}
                    </li>
                    <li>
                      {{
                        t(
                          ...
                          '每个维度先与所有候选目标横向对比，给出 0–100 的相对得分。',
                        )
                      }}
                    </li>
                    <li>
                      {{
                        t(
                          ...
                          '再按你选择的目标偏好（均衡 / 收益优先 / 经验优先 / 自定义权重）加权汇总成“基础分”。',
                        )
                      }}
                    </li>
                    <li>
                      {{ t('common:advisor.scoreExplainComposite4', '最终综合分 = 基础分 × (0.85 + 0.15 × 置信度)。') }}
                    </li>
                  </ul>
                </div>
                <div>
                  <p class="font-semibold text-foreground">
                    {{ t('common:advisor.scoreExplainConfidenceHeading', '置信度怎么算？') }}
                  </p>
                  <ul class="mt-1 list-disc space-y-1 pl-5">
                    <li>
                      {{
                        t(
                          ...
                          '反映“多次扫描结果是否一致”——多次跑出来的数据越接近，置信度越高。',
                        )
                      }}
                    </li>
                    <li>
                      {{
                        t(
                          ...
                          '样本数量：扫描轮数越多越高，复核 10–20 轮即可接近上限。',
                        )
                      }}
                    </li>
                    <li>
                      {{
                        t('common:advisor.scoreExplainConfidence3', '轮间波动：每轮收益、经验、死亡数据差距越小越高。')
                      }}
                    </li>
                    <li>
                      {{
                        t(
                          ...
                          '置信度对最终分影响较小（仅约 15%），即便置信度为 0，最终分仍保留基础分的 85%。',
                        )
                      }}
                    </li>
                  </ul>
...
const runtimePhaseText = computed(() => {
  ...
  if (phase === 'quick_scan') {
    return (
      ...
      ` · ${runtime.value.quickCompleted || 0}/${runtime.value.quickTotal || 0}`
    );
  }
  if (phase === 'refine_top') {
    return (
      t('common:advisor.phaseRefine', 'Refining top picks') +
      ` · ${runtime.value.refineCompleted || 0}/${runtime.value.refineTotal || 0}`
    );
  }
  if (phase === 'done') {
