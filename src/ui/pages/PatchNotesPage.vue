<template>
  <section class="mx-auto max-w-5xl space-y-4">
          ...
          <p class="mt-2 max-w-3xl text-sm leading-6 text-foreground/85">
            {{
              t(
                ...
                'Review simulator changes, fixes, and interaction updates by version.',
              )
            }}
          </p>
          <p class="mt-2 text-xs text-muted-foreground">
            {{
              t(
                ...
                'Unread patch notes are marked as read when you open this page.',
              )
            }}
          </p>
        </div>
    ...
    <div v-if="entries.length > 0" class="overflow-hidden rounded-lg border border-border bg-card text-card-foreground">
      <article
