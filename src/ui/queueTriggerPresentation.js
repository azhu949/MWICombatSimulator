...
export function formatQueueTriggerLabel(targetHrid, resolveTargetName, translate) {
  ...
  const resolvedName = typeof resolveTargetName === 'function' ? String(resolveTargetName(hrid) || '').trim() : '';
  if (!resolvedName) {
    return t('common:queue.triggerLabel', 'Trigger');
  }
...
export function formatQueueTriggerDetailLine(change, options = {}) {
  const targetHrid = normalizeTriggerTargetHrid(change?.targetHrid || parseTriggerTargetHrid(change?.label));
  ...
  const normalizedBeforeState = String(beforeState || 'default')
    .trim()
    .toLowerCase();
  const normalizedAfterState = String(afterState || 'default')
    .trim()
    .toLowerCase();
