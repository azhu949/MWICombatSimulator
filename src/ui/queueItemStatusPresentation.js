...
function normalizeResolver(resolver, fallbackValue = '-') {
  return typeof resolver === 'function' ? resolver : (value) => String(value || fallbackValue);
}
...
function formatChangeValue(change, side, options) {
  ...
  if (kind === 'ability') {
    ...
    const level = side === 'before' ? toAbilityLevelText(change?.beforeLevel) : toAbilityLevelText(change?.afterLevel);
