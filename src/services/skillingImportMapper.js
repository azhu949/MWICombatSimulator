...
function normalizeDrinkSlotMap(rawMap) {
  ...
  for (const [rawActionType, rawSlots] of Object.entries(isPlainObject(rawMap) ? rawMap : {})) {
    ...
    const slots = toValueList(rawSlots)
      .map((rawSlot, index) => {
        const itemHrid =
        ...
        return {
          ...
          itemHrid,
          isActive:
            rawSlot?.isActive === true || rawSlot?.active === true
              ? true
...
function guildBuffMaximumLevel(detail) {
  const shrineMaximum = Math.max(0, Math.trunc(finiteNumber(guildShrineDetailMap?.[detail?.shrineHrid]?.maxLevel, 0)));
  const costMaximum = Math.max(
    0,
    ...Object.keys(detail?.levelCosts || {}).map((level) => Math.trunc(finiteNumber(level, 0))),
  );
...
function buildGuildActionBuffMap(source) {
  ...
  for (const detail of Object.values(guildBuffDetailMap || {})) {
    ...
    const effectiveLevel = Math.min(purchasedLevels[detail.hrid] || 0, shrineLevel, guildBuffMaximumLevel(detail));
    if (effectiveLevel <= 0) continue;
    for (const template of detail?.buffs || []) {
      const normalized = normalizeBuff({
        ...template,
        ratioBoost:
          finiteNumber(template?.ratioBoost, 0) +
          finiteNumber(template?.ratioBoostLevelBonus, 0) * (effectiveLevel - 1),
        flatBoost:
          finiteNumber(template?.flatBoost, 0) + finiteNumber(template?.flatBoostLevelBonus, 0) * (effectiveLevel - 1),
        duration: 0,
      });
  ...
  return Object.fromEntries(
    (skillingData?.actionTypeHrids || []).map((actionTypeHrid) => [actionTypeHrid, buffs.map((buff) => ({ ...buff }))]),
  );
}
...
export function normalizeSkillingProfile(payload, importedAt = Date.now()) {
  ...
  for (const rawEntry of toValueList(source.characterItems)) {
    ...
    const equipmentType = normalizeHrid(
      equipmentDefinition?.equipmentType || itemDefinition?.equipmentType || itemDefinition?.equipmentDetail?.type,
    );
    ...
    if (itemDefinition?.categoryHrid === '/item_categories/equipment' && (isInInventory || isEquipped)) {
      equipment.push({
        ...
        enhancementLevel: Math.max(
          0,
          Math.min(20, Math.trunc(finiteNumber(entry?.enhancementLevel ?? rawEntry?.enhancementLevel, 0))),
        ),
        count,
        isEquipped,
      });
