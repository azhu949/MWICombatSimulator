...
const DEFAULT_SUCCESS_RATES = [
  0.5, 0.45, 0.45, 0.4, 0.4, 0.4, 0.35, 0.35, 0.35, 0.35, 0.3, 0.3, 0.3, 0.3, 0.3, 0.3, 0.3, 0.3, 0.3, 0.3,
];
...
function normalizeMarkupRate(config) {
  ...
  return toNonNegativeNumber(config.markupPercent ?? config.percentRate ?? config.percent_rate, 0) / 100;
}
...
export function normalizeEnhancementConfig(rawConfig = {}, enhancementData = {}) {
  ...
  const rawCommunitySpeedLevel =
    rawConfig.communitySpeedLevel ?? rawConfig.communityEnhancingLevel ?? rawConfig.enhancingCommunityBuffLevel;
  const rawCommunityExperienceLevel = rawConfig.communityExperienceLevel ?? rawConfig.experienceCommunityBuffLevel;
  ...
  return {
    ...
    otherRoomLevelsTotal: Math.max(0, toInteger(rawConfig.otherRoomLevelsTotal ?? rawConfig.otherRoomLevels, 0)),
    houseRoomLevels:
      rawConfig.houseRoomLevels && typeof rawConfig.houseRoomLevels === 'object'
    ...
    communitySpeedLevel: clamp(toInteger(rawCommunitySpeedLevel, 1), 1, 20),
    ...
    communityExperienceLevel: clamp(toInteger(rawCommunityExperienceLevel, 1), 1, 20),
    ...
    equipment: normalizeEquipmentSelections(
      rawConfig.equipment ?? rawConfig.equipmentSlots ?? rawConfig.supportEquipment ?? rawConfig.equipmentSelections,
    ),
    enhancingTeaHrid: teaHrid,
    blessedEnabled: normalizeBoolean(
      rawConfig.blessedEnabled ?? rawConfig.blessedTeaEnabled ?? rawConfig.blessedTea ?? rawConfig.teaBlessed,
      false,
    ),
    wisdomEnabled: normalizeBoolean(
      rawConfig.wisdomEnabled ?? rawConfig.wisdomTeaEnabled ?? rawConfig.wisdomTea ?? rawConfig.teaWisdom,
      false,
    ),
    drinkConcentrationMultiplier:
      explicitConcentration == null ? null : Math.max(1, toFiniteNumber(explicitConcentration, 1)),
    ...
    priceOverrides:
      rawConfig.priceOverrides && typeof rawConfig.priceOverrides === 'object' ? { ...rawConfig.priceOverrides } : {},
    startingItemPriceOverride:
      rawConfig.startingItemPriceOverride == null ? null : toNonNegativeNumber(rawConfig.startingItemPriceOverride, 0),
...
export function resolveBuffValue(buff, level = 1, channel = 'flat') {
  ...
  return toFiniteNumber(buff[baseKey], 0) + (normalizedLevel - 1) * toFiniteNumber(buff[levelKey], 0);
}
...
export function resolveEnhancedStat(equipment, statName, enhancementLevel, totalBonusMultipliers) {
  ...
  const multipliers = Array.isArray(totalBonusMultipliers) ? totalBonusMultipliers : DEFAULT_TOTAL_BONUS_MULTIPLIERS;
  const multiplier = toFiniteNumber(multipliers[level], 0);
  const stats = equipment.stats || equipment.noncombatStats || equipment.equipmentDetail?.noncombatStats || {};
  const enhancementBonuses =
    equipment.enhancementBonuses ||
    equipment.noncombatEnhancementBonuses ||
    equipment.equipmentDetail?.noncombatEnhancementBonuses ||
    {};
  return toFiniteNumber(stats[statName], 0) + toFiniteNumber(enhancementBonuses[statName], 0) * multiplier;
}
...
export function resolveEquipmentStats(config, enhancementData = {}) {
  ...
  for (const selection of normalizedConfig.equipment || []) {
    ...
    for (const statName of statNames) {
      totals[statName] =
        toFiniteNumber(totals[statName], 0) +
...
function drinkDurationSeconds(drink) {
  const directDuration = toFiniteNumber(drink?.durationSeconds ?? drink?.buffDurationSeconds, 0);
  if (directDuration > 0) {
  ...
  const buffDurations = buffsForDrink(drink)
    .map((buff) => {
      ...
      return normalized > 0 ? normalized : toFiniteNumber(buff?.duration, 0) / NANOSECONDS_PER_SECOND;
    })
    .filter((duration) => duration > 0);
  return buffDurations.length > 0 ? Math.min(...buffDurations) : DEFAULT_DRINK_DURATION_SECONDS;
}
...
function selectedEnhancementDrinks(config, enhancementData) {
  ...
  if (config.wisdomEnabled) {
    selected.push(findByHrid(drinks, '/items/wisdom_tea') || findByHrid(drinks, '/items/wisdom_coffee'));
  }
  return Array.from(
    new Map(
      selected.filter((drink) => normalizeHrid(drink?.hrid)).map((drink) => [normalizeHrid(drink.hrid), drink]),
    ).values(),
  );
}
...
function resolveHouseWisdom(config, enhancementData) {
  ...
  for (const [roomHrid, rawLevel] of Object.entries(config.houseRoomLevels || {})) {
    ...
    const buff =
      compactGlobalExperience.buff ||
      findBuff(room?.globalBuffs || room?.buffs || room, (candidate) => typeIncludes(candidate, 'wisdom'));
  ...
  if (aggregateLevels > 0) {
    ...
    const buff =
      compactGlobalExperience.buff ||
      findBuff(representative?.globalBuffs || representative?.buffs || representative, (candidate) =>
        typeIncludes(candidate, 'wisdom'),
      );
    if (buff) {
...
export function resolveEnhancementBonuses(rawConfig, enhancementData = {}) {
  ...
  const concentration =
    config.drinkConcentrationMultiplier == null
  ...
  const skillSuccessRatio =
    effectiveSkillLevel >= itemLevel
  ...
  const observatorySuccessBuff = findBuff(observatory.actionBuffs, (buff) => typeIncludes(buff, 'enhancing_success'));
  const observatorySpeedBuff = findBuff(observatory.actionBuffs, (buff) => typeIncludes(buff, 'action_speed'));
  ...
  const championBuff = config.championAchievement
    ...
    : null;
  const championSuccessRatio = resolveBuffValue(championBuff, 1, 'ratio') + resolveBuffValue(championBuff, 1, 'flat');
  const equipmentSuccessRatio = toFiniteNumber(equipmentStats.enhancingSuccess, 0);
  const totalSuccessRatio = skillSuccessRatio + observatorySuccessRatio + championSuccessRatio + equipmentSuccessRatio;

  const skillSpeed = Math.max(0, (effectiveSkillLevel - itemLevel) / 100);
  const equipmentSpeed =
    toFiniteNumber(equipmentStats.enhancingSpeed, 0) +
  ...
  const wisdomDrink = findByHrid(drinks, '/items/wisdom_tea') || findByHrid(drinks, '/items/wisdom_coffee');
  ...
  const communityExperienceBuff = config.communityExperienceEnabled
    ...
    : null;
  const communityWisdom = resolveBuffValue(communityExperienceBuff, config.communityExperienceLevel, 'flat');
  const noviceBuff = config.noviceAchievement ? findAchievementBuff(enhancementData, 'novice', 'wisdom') : null;
  const achievementWisdom = resolveBuffValue(noviceBuff, 1, 'flat');
  const equipmentWisdom =
    toFiniteNumber(equipmentStats.enhancingExperience, 0) +
...
function normalizeSuccessRates(enhancementData) {
  const source = Array.isArray(enhancementData?.successRates) ? enhancementData.successRates : DEFAULT_SUCCESS_RATES;
...
export function buildEnhancementTransitionModel(rawConfig, enhancementData = {}) {
  ...
  for (let currentLevel = 0; currentLevel < stateCount; currentLevel += 1) {
    ...
    const failureLevel =
      config.protectAt != null && currentLevel >= config.protectAt ? Math.max(0, currentLevel - 1) : 0;
...
function priceEntryValue(entry, source) {
  ...
  const rawValue = source === 'ask' ? (entry.ask ?? entry.a) : source === 'bid' ? (entry.bid ?? entry.b) : entry.vendor;
...
function valueFromPriceEntry(entry, mode, requestedSources = null, overrideEntry = null) {
  const defaultSources = mode === 'bid' ? ['bid', 'ask', 'vendor'] : ['ask', 'bid', 'vendor'];
  const sources =
...
function resolveOverride(overrides, itemHrid, level) {
  ...
  const levelKeys = level == null ? [] : [`${itemHrid}:${level}`, `${itemHrid}@${level}`];
...
export function resolveEnhancementPrice(pricing = {}, itemHrid, options = {}) {
  ...
  const objectOverride =
    ...
    (override.ask != null || override.bid != null || override.a != null || override.b != null)
      ? override
      : null;
  ...
  if (level != null) {
    const quote =
      pricing.enhancementQuotesByItem?.[hrid]?.[String(level)] || pricing.enhancementQuotes?.[hrid]?.[String(level)];
    if (quote) {
      const resolved = valueFromPriceEntry(quote, mode, options.sources, objectOverride);
      return {
        ...resolved,
        source:
          resolved.available && resolved.source !== 'override' ? `enhancement_${resolved.source}` : resolved.source,
        itemHrid: hrid,
        level,
      };
    }
...
function enhancementCatalogItem(enhancementData, itemHrid) {
  return (
    findByHrid(enhancementData?.enhanceableItems, itemHrid) ||
...
export function estimateEnhancementAcquisitionValue(itemHrid, enhancementData = {}, pricing = {}, options = {}) {
  ...
  const sources = Array.isArray(acquisition?.sourcesByItemHrid?.[hrid]) ? acquisition.sourcesByItemHrid[hrid] : [];
  ...
  for (const source of sources) {
    ...
    const containerExpectedCount =
      (Math.max(0, toFiniteNumber(source.containerDropRate, 0)) *
        (Math.max(0, toFiniteNumber(source.containerMinCount, 0)) +
          Math.max(0, toFiniteNumber(source.containerMaxCount, 0)))) /
      2;
    const targetDrops =
      Array.isArray(source.itemDrops) && source.itemDrops.length > 0
        ? source.itemDrops
        : (openable.drops || []).filter((drop) => normalizeHrid(drop?.itemHrid) === hrid);
    const targetExpectedCountPerContainer = targetDrops.reduce((sum, drop) => sum + expectedDropCount(drop), 0);
...
export function resolveProtectionCandidates(rawConfig, enhancementData = {}, pricing = {}) {
  ...
  candidates.sort((left, right) => {
    ...
    return (
      toFiniteNumber(left.price, Infinity) - toFiniteNumber(right.price, Infinity) ||
...
function calculateCostContext(config, model, enhancementData, pricing, expectedActions, expectedProtections) {
  ...
  const concentration = Math.max(1, toFiniteNumber(model?.bonuses?.drinkConcentrationMultiplier, 1));
  ...
  const rawIncrementalCost = incrementalCostAvailable
    ? expectedActions * (stepCost + laborPerAction) + expectedProtections * toNonNegativeNumber(protectionCost, 0)
    : null;
  ...
  return {
    ...
    totalInvestment:
      startingItem.available && incrementalCostAvailable
...
export function analyzeEnhancementStrategy(rawConfig, enhancementData = {}, pricing = {}) {
  ...
  const cost = calculateCostContext(config, model, enhancementData, pricing, expectedActions, expectedProtections);
...
export function analyzeEnhancementStrategies(rawConfig, enhancementData = {}, pricing = {}) {
  ...
  const strategies = thresholds.map((protectAt) =>
    analyzeEnhancementStrategy({ ...config, protectAt }, enhancementData, pricing),
  );
  const sortableCost = (strategy) => {
    ...
    return incrementalCost != null && Number.isFinite(Number(incrementalCost)) ? Number(incrementalCost) : Infinity;
  };
  const recommendedStrategy = strategies.reduce(
    (best, strategy) => (!best || sortableCost(strategy) < sortableCost(best) ? strategy : best),
    null,
  );
  return {
    config,
...
function rewardResolver(reward) {
  ...
  return (outcome) =>
    toFiniteNumber(descriptor.perAction, 0) +
...
export function calculateStrategyCostMoments(strategy) {
  ...
  const initial = context?.startingItem?.available ? toNonNegativeNumber(context.startingItem.price, 0) * markup : 0;
...
function normalizePercentiles(percentiles) {
  const source = Array.isArray(percentiles) && percentiles.length > 0 ? percentiles : ENHANCEMENT_PERCENTILES;
...
export function fitGammaRisk(moments, options = {}) {
  ...
  const budgetProbability =
    budget == null ? null : budget <= offset ? 0 : regularizedGammaP(shape, (budget - offset) / scale);
  return {
    method: 'moment_gamma',
    approximate: true,
    mean,
  ...
  const rng =
    typeof rngOrOptions === 'function'
      ...
      : typeof rngOrOptions?.random === 'function'
        ? rngOrOptions.random
        : Math.random;
  const options = typeof rngOrOptions === 'function' ? maybeOptions || {} : rngOrOptions || {};
...
function resolveTrialCostParameters(context, options = {}) {
  ...
  const startingItemCost =
    options.startingItemCost != null
...
function buildCompletedTrial(model, costParameters, actions, protections, experience, finalLevel) {
  const { markup, stepCost, laborPerAction, protectionCost, startingItemCost, materialCostPerAction, coinPerAction } =
    costParameters;
...
export function runEnhancementTrial(input, rngOrOptions, maybeOptions) {
  ...
  return buildCompletedTrial(model, costParameters, actions, protections, experience, currentLevel);
}
...
export function summarizeMonteCarloTrials(trials, options = {}) {
  ...
  const budget = options.budget == null ? null : toNonNegativeNumber(options.budget, 0);
  const budgetProbability =
    budget == null || validTrials.length === 0
      ? null
      : validTrials.filter((trial) => trial.totalCost <= budget).length / validTrials.length;
  const mean =
    validTrials.length === 0 ? 0 : validTrials.reduce((sum, trial) => sum + trial.totalCost, 0) / validTrials.length;
  const variance =
...
export function createMonteCarloTrialAccumulator(input, options = {}) {
  ...
  function summarize(summaryOptions = options) {
    ...
    const budget = summaryOptions.budget == null ? null : toNonNegativeNumber(summaryOptions.budget, 0);
    let total = 0;
    let budgetMatches = 0;
    for (let index = 0; index < length; index += 1) {
  ...
  return {
    ...
    get storageByteLength() {
      return totalCosts.byteLength + actionCounts.byteLength + protectionCounts.byteLength + experiences.byteLength;
    },
  };
}
...
export function runMonteCarloTrials(input, options = {}) {
  ...
  const hardTransitionLimit = Math.max(1, toInteger(options.hardTransitionLimit, MONTE_CARLO_HARD_TRANSITION_LIMIT));
  const includeTrials = options.includeTrials !== false;
  const trials = includeTrials ? [] : null;
  const compactTrials = includeTrials ? null : createMonteCarloTrialAccumulator(input, { ...options, sampleCount });
  let completed = 0;
  let transitions = 0;
  let exceeded = false;
  let cancelled = false;
  ...
  const summary = includeTrials ? summarizeMonteCarloTrials(trials, options) : compactTrials.summarize(options);
  return {
    ...summary,
    seed,
    sampleCount,
...
export function analyzeEnhancementRisk(strategy, options = {}) {
  ...
  const initialOffset = strategy.startingItem?.available ? strategy.startingItem.price * strategy.markupMultiplier : 0;
...
function requirementCounts(requirements) {
  return Object.fromEntries(Object.entries(requirements || {}).map(([key, requirement]) => [key, requirement.count]));
}
...
export function planPhilosophersMirror(rawPlans, rawOptions = {}) {
  ...
  const mirrorPrice = rawMirrorPrice === Infinity ? Infinity : toNonNegativeNumber(rawMirrorPrice, 0);
  ...
  for (let level = 0; level <= targetLevel; level += 1) {
    ...
    const lowerBasePlan = level >= 2 ? basePlan?.levels?.[level - 2] || levels[level - 2] : null;
    if (level >= 2 && lowerBasePlan && levels[level - 1]) {
      const mirrorCost = lowerBasePlan.cost + levels[level - 1].cost + mirrorPrice + mirrorActionCost;
      if (mirrorCost < directCost) {
        ...
        selected = {
          ...
          mirrorItemCost: lowerBasePlan.mirrorItemCost + levels[level - 1].mirrorItemCost + mirrorPrice,
          mirrorLaborCost: lowerBasePlan.mirrorLaborCost + levels[level - 1].mirrorLaborCost + mirrorActionCost,
...
export function calculateDecompositionValue(input = {}, enhancementData = {}, pricing = {}) {
  const item = input.item || findByHrid(enhancementData?.enhanceableItems, input.itemHrid) || {};
  ...
  const essenceCount = Math.floor(Math.round(2 * (0.5 + 0.1 * 1.05 ** itemLevel) * 2 ** targetLevel));
