...
const UNSUPPORTED_SKILLING_BUFF_TYPE_HRIDS = new Set(['/buff_types/processing']);
const GATHERING_SKILL_KEYS = new Set(['milking', 'foraging', 'woodcutting']);
...
export function normalizeSkillingBalancedCostTolerance(value) {
  ...
  return Number.isFinite(normalizedValue) ? clamp(normalizedValue, 0, 1) : SKILLING_BALANCED_COST_TOLERANCE;
}
...
function addBuff(totals, buff, skillHrid, multiplier = 1) {
  ...
  else if (typeHrid === '/buff_types/gathering' && GATHERING_SKILL_KEYS.has(skillKeyFromHrid(skillHrid)))
    totals.outputQuantity += value;
  else if (typeHrid === '/buff_types/processing' && GATHERING_SKILL_KEYS.has(skillKeyFromHrid(skillHrid)))
    totals.processing += value;
  else if (typeHrid === '/buff_types/wisdom' || typeHrid === '/buff_types/skilling_experience')
    totals.experience += value;
...
export function collectSkillingProfileBonuses(profile, actionTypeHrid, skillHrid, now = Date.now()) {
  ...
  for (const sourceMap of Object.values(profile?.buffsBySource || {})) {
    ...
    for (const buff of buffs) {
      ...
      if (applied && expirationAt != null && (nextExpirationAt == null || expirationAt < nextExpirationAt)) {
...
export function resolveSkillingPrice(
  ...
  const enhancedEntry =
    normalizedEnhancementLevel > 0 ? enhancementQuotesByItem?.[hrid]?.[String(normalizedEnhancementLevel)] : null;
  ...
  const liquidationSource =
    marketLiquidation >= vendor && marketLiquidation > 0 ? (bid >= 0 ? 'market_bid' : 'base_bid_floor') : 'vendor';
  return {
    itemHrid: hrid,
    ask,
    bid,
...
function equipmentRequirementsMet(definition, skillLevels) {
  return (definition?.levelRequirements || []).every(
    (requirement) =>
...
function equipmentBonusesForSkill(stats, skillHrid) {
  ...
  return {
    actionSpeed:
      finiteNumber(stats?.[`${key}Speed`], 0) +
      finiteNumber(stats?.skillingSpeed, 0) +
      finiteNumber(stats?.actionSpeed, 0),
    efficiency: finiteNumber(stats?.[`${key}Efficiency`], 0) + finiteNumber(stats?.skillingEfficiency, 0),
    outputQuantity: GATHERING_SKILL_KEYS.has(key) ? finiteNumber(stats?.gatheringQuantity, 0) : 0,
    experience:
      finiteNumber(stats?.[`${key}Experience`], 0) +
      finiteNumber(stats?.skillingExperience, 0) +
      finiteNumber(stats?.wisdom, 0),
    essenceFind: finiteNumber(stats?.skillingEssenceFind, 0),
    rareFind: finiteNumber(stats?.[`${key}RareFind`], 0) + finiteNumber(stats?.skillingRareFind, 0),
...
function stateStableSignature(state) {
  return (state?.items || [])
    .map((item) => `${item.equipmentType}:${item.itemHrid}:${item.enhancementLevel}`)
    .join('|');
}

function stateEquipmentReservationSignature(state, reservationSensitiveHrids = null) {
  const sensitiveHrids = reservationSensitiveHrids instanceof Set ? reservationSensitiveHrids : null;
  const countsByInstanceType = {};
  for (const item of state?.items || []) {
    const itemHrid = normalizeHrid(item?.itemHrid);
    if (!sensitiveHrids || sensitiveHrids.has(itemHrid)) {
...
function stateDominates(left, right, reservationSensitiveHrids, experienceDirection) {
  ...
  strictlyBetter ||=
    experienceDirection > 0 ? leftExperience > rightExperience + EPSILON : leftExperience + EPSILON < rightExperience;
...
function pruneEquipmentStates(states, reservationSensitiveHrids) {
  ...
  for (const candidate of candidates) {
    const dominatedForHigherExperience = candidates.some(
      (other) => other !== candidate && stateDominates(other, candidate, reservationSensitiveHrids, 1),
    );
    const dominatedForLowerExperience = candidates.some(
      (other) => other !== candidate && stateDominates(other, candidate, reservationSensitiveHrids, -1),
    );
...
export function buildSkillingEquipmentLoadouts(
  ...
  const owned = (profile?.equipment || [])
    .map((instance) => {
      ...
      const isAvailable =
        finiteNumber(instance?.count, 1) >= 1 - EPSILON &&
        (!hasInventoryLedger || finiteNumber(inventory?.[instance?.itemHrid], 0) >= 1 - EPSILON);
  ...
  for (const item of owned) {
    ...
    if (
      !options.some(
        (candidate) => `${candidate.itemHrid}:${candidate.enhancementLevel}:${candidate.isEquipped ? 1 : 0}` === key,
      )
    ) {
      options.push(item);
    }
  ...
  for (const [equipmentType, rawOptions] of byType.entries()) {
    ...
    for (const state of states) {
      for (const option of options) {
        ...
        const selectionChanged = equipped ? option?.id !== equipped.id : optionSelected;
        nextStates.push({
...
function buildDrinkLoadouts(data, actionTypeHrid, maximumSlots, blockedUniqueHrids = []) {
  ...
  const drinks = (data?.drinks || [])
    .filter(
      (drink) =>
        ...
        !(drink?.buffs || []).some((buff) => UNSUPPORTED_SKILLING_BUFF_TYPE_HRIDS.has(normalizeHrid(buff?.typeHrid))),
    )
    .sort(
      (left, right) =>
  ...
  function visit(startIndex, selected, selectedUniqueHrids) {
    ...
    for (let index = startIndex; index < drinks.length; index += 1) {
      ...
      const uniqueHrids = Array.from(
        new Set((drink?.buffs || []).map((buff) => normalizeHrid(buff?.uniqueHrid)).filter(Boolean)),
      );
      if (
        uniqueHrids.some((uniqueHrid) => blockedUniqueHridSet.has(uniqueHrid) || selectedUniqueHrids.has(uniqueHrid))
      ) {
        continue;
      }
...
function expectedDropCount(drop) {
  return (
    (Math.max(0, finiteNumber(drop?.dropRate, 0)) *
...
function createOwnedItemPools(inventory, equipmentInstances, selectedEquipment, reusableGeneratedItems = {}) {
  const pools = new Map(
    Object.entries(inventory || {}).map(([itemHrid, rawCount]) => [
      itemHrid,
      {
        itemHrid,
  ...
  for (const instance of equipmentInstances || []) {
    ...
    const representedCount = Math.min(pool.genericCount, Math.max(0, finiteNumber(instance?.count, 1)));
...
function allocateOwnedRequirement({
  ...
  const basePurchaseQuote = resolveSkillingPrice(priceTable, pool?.itemHrid, feeRate, enhancementQuotesByItem, 0);
  ...
  const candidates = (pool?.sources || [])
    ...
    .map((source) => {
      ...
      const retainedValue =
        typeof retainedOutputValue === 'function' ? finiteNumber(retainedOutputValue(source.enhancementLevel), 0) : 0;
    ...
    .filter((source) => {
      ...
      return (
        basePurchaseQuote.purchasePrice == null ||
  ...
  for (const candidate of candidates) {
    ...
    const source = pool.sources.find(
      (entry) =>
...
function buildLedgerRows({
  ...
  const pools = createOwnedItemPools(inventory, equipmentInstances, selectedEquipment, reusableGeneratedItems);
  ...
  function retainedOutputValue(enhancementLevel) {
    return (actionOutputItems || []).reduce((sum, output) => {
      ...
      return (
        sum +
        Math.max(0, finiteNumber(output?.count, 0)) *
  ...
  function recordRequirement(itemHrid, rawCount, options = {}) {
    ...
    const ownedValue = allocation.sources.reduce(
      (sum, source) => sum + source.count * source.quote.liquidationPrice,
      0,
    );
  ...
  if (retainAllEnhancement === true && upgradeAllocation) {
    ...
    for (const output of actionOutputItems || []) {
      outputEntries.push({
        itemHrid: output?.itemHrid,
        count:
          finiteNumber(output?.count, 0) *
          Math.max(0, finiteNumber(upgradeCount, 0)) *
          Math.max(0, finiteNumber(outputMultiplier, 1)),
        enhancementLevel: 0,
      });
    }
  ...
  for (const entry of outputEntries) {
    ...
    if (!itemHrid || count <= EPSILON) continue;
    const quote = resolveSkillingPrice(priceTable, itemHrid, feeRate, enhancementQuotesByItem, enhancementLevel);
...
function applyPlanningEquipmentChanges({
  ...
  const consumedById = new Map(
    (consumedEquipment || []).map((item) => [String(item?.id || ''), finiteNumber(item?.count, 0)]),
  );
  ...
  const equipmentItemHrids = new Set([...(data?.equipmentItemHrids || []), ...supportDefinitions.keys()]);
  const next = [];
  for (const instance of equipmentInstances || []) {
    const remainingCount = Math.max(
      0,
      finiteNumber(instance?.count, 1) - finiteNumber(consumedById.get(String(instance?.id || '')), 0),
    );
    ...
    next.push({
      ...
      itemLocationHrid: isEquipped
        ? `/item_locations/${
            String(supportDefinition?.equipmentType || instance?.equipmentType || '')
              .split('/')
              .filter(Boolean)
              .pop() || 'inventory'
          }`
...
function drinkBonuses(loadout, skillHrid, concentrationByItem) {
  ...
  for (const drink of loadout) {
    ...
    const concentrationMultiplier = Math.max(1, finiteNumber(concentrationByItem?.[itemHrid], 1));
...
export function calculateSkillingActionCandidate({
  ...
  const equipmentConcentrationMultiplier = Math.max(1, 1 + finiteNumber(equipmentLoadout?.drinkConcentration, 0));
  ...
  for (const drink of drinkLoadout) {
    ...
    concentrationByItem[itemHrid] =
      finiteNumber(activeState?.remainingSeconds, 0) > EPSILON
  ...
  const suppliedStartWindow =
    actionStartWindowSeconds == null ? null : Math.max(0, finiteNumber(actionStartWindowSeconds, 0));
  const requestedRenewDrinkHrids = new Set(
    Array.from(renewDrinkHrids || [])
      .map(normalizeHrid)
      .filter(Boolean),
  );
  ...
  const drinkStartWindow = drinkLoadout.reduce((minimum, drink) => {
    const itemHrid = normalizeHrid(drink?.hrid);
    const activeState = drinkState?.itemsByHrid?.[itemHrid];
    const carriedSeconds = Math.max(0, finiteNumber(activeState?.remainingSeconds, 0));
    const concentrationMultiplier = Math.max(
      1,
      finiteNumber(concentrationByItem[itemHrid], equipmentConcentrationMultiplier),
    );
    const concentrationMatches =
      carriedSeconds <= EPSILON ||
      Math.abs(Math.max(1, finiteNumber(activeState?.concentrationMultiplier, 1)) - equipmentConcentrationMultiplier) <=
        EPSILON;
  ...
  const normalizedStartWindow = [suppliedStartWindow, drinkStartWindow]
    .filter((value) => value != null && Number.isFinite(value))
    .reduce((minimum, value) => Math.min(minimum, value), Infinity);
  const effectiveStartWindow = Number.isFinite(normalizedStartWindow) ? normalizedStartWindow : null;
  const windowCompletionCount =
    effectiveStartWindow == null
  ...
  for (const drop of action?.essenceDropTable || []) {
    addAmount(
      outputItems,
      drop?.itemHrid,
      expectedDropCount(drop) * completionCount * Math.max(0, 1 + bonuses.essenceFind),
    );
  }
  for (const drop of action?.rareDropTable || []) {
    addAmount(
      outputItems,
      drop?.itemHrid,
      expectedDropCount(drop) * completionCount * Math.max(0, 1 + bonuses.rareFind),
    );
  }
  ...
  for (const drink of drinkLoadout) {
    ...
    const effectiveDuration = finiteNumber(drink?.durationSeconds, 300) / concentrationMultiplier;
    const carriedSeconds = Math.max(0, finiteNumber(drinkState?.itemsByHrid?.[itemHrid]?.remainingSeconds, 0));
    const carriedCompletionCount =
      carriedSeconds > EPSILON ? Math.max(1, Math.ceil(carriedSeconds / actionIntervalSeconds - EPSILON)) : 0;
    const completionCountPerDrink = Math.max(1, Math.ceil(effectiveDuration / actionIntervalSeconds - EPSILON));
    ...
    const lastDrinkStartSeconds =
      count > 0 ? (carriedCompletionCount + (count - 1) * completionCountPerDrink) * actionIntervalSeconds : null;
    const endingRemainingSeconds =
      count > 0
  ...
  const canReuseCompletedCycles =
    reuseCompletionInterval > 0 &&
    completionCount > reuseCompletionInterval &&
    completionCount % reuseCompletionInterval === 0;
  const finalReuseChunkCompletionCount =
    reuseCompletionInterval > 0
      ? completionCount % reuseCompletionInterval || reuseCompletionInterval
      : completionCount;
  const initialReuseChunkCompletionCount =
    reuseCompletionInterval > 0 ? Math.min(completionCount, reuseCompletionInterval) : completionCount;
  ...
  const drinkPurchaseCost = Math.max(
    0,
    Math.min(
      ledger.purchaseCost,
  ...
  return {
    ...
    drinks: drinkRows,
    endingDrinkState:
      Object.keys(endingDrinkItemsByHrid).length > 0
        ? {
...
function candidateDrinkCycleCount(candidate) {
  return Math.max(0, ...(candidate?.drinks || []).map((drink) => Math.max(0, integer(drink?.count, 0))));
}
...
function simulationDrinkCycleCount(simulation) {
  ...
  return (simulation?.segments || []).reduce((count, segment) => count + candidateDrinkCycleCount(segment), 0);
}

function compareCandidateStableKeys(left, right) {
  return (
    finiteNumber(left?.actionSortIndex, 0) - finiteNumber(right?.actionSortIndex, 0) ||
    String(left?.actionHrid || '').localeCompare(String(right?.actionHrid || '')) ||
    String(left?.drinks?.map((drink) => drink.itemHrid).join('|') || '').localeCompare(
      String(right?.drinks?.map((drink) => drink.itemHrid).join('|') || ''),
    )
  );
}
...
function compareBalancedCandidates(
  ...
  const normalizedLowestCost = lowestCostPerExperience == null ? NaN : Number(lowestCostPerExperience);
  ...
  const maximumBalancedCost = normalizedLowestCost + Math.abs(normalizedLowestCost) * normalizedBalancedCostTolerance;
  ...
  const costDelta = leftCost - rightCost;
  const durationDelta = leftDuration - rightDuration;
  const materialCostDelta =
    finiteNumber(left?.materialPurchaseCostPerExperience, Infinity) -
...
function compareCandidates(
  ...
  if (normalizedOptimizationMode === SKILLING_OPTIMIZATION_MODE_BALANCED) {
    return compareBalancedCandidates(left, right, balancedLowestCostPerExperience, balancedCostTolerance);
  }
...
function rankCandidates(
  ...
  const normalizedOptimizationMode = normalizeSkillingOptimizationMode(optimizationMode);
  const normalizedBalancedCostTolerance = normalizeSkillingBalancedCostTolerance(balancedCostTolerance);
  const suppliedBalancedBaseline =
    balancedLowestCostPerExperience == null || balancedLowestCostPerExperience === ''
      ? NaN
      : Number(balancedLowestCostPerExperience);
  const lowestCostPerExperience =
    normalizedOptimizationMode !== SKILLING_OPTIMIZATION_MODE_BALANCED
      ...
      : Number.isFinite(suppliedBalancedBaseline)
        ...
        : (candidates || []).reduce(
            (minimum, candidate) => Math.min(minimum, finiteNumber(candidate?.costPerExperience, Infinity)),
            Infinity,
          );
...
function stableOwnedInputCycleLimit(candidate, inventory, equipmentInstances) {
  ...
  const equipmentHrids = new Set(
    (equipmentInstances || []).map((item) => normalizeHrid(item?.itemHrid)).filter(Boolean),
  );
  ...
  for (const [itemHrid, totals] of inputsByHrid) {
    ...
    if (outputCanBeReused && sameItemOutputCount >= totals.count - EPSILON) {
      ...
      cyclesBeforeDepletion = Math.floor(Math.max(0, availableCount - totals.count + EPSILON) / netConsumption) + 1;
    } else {
      cyclesBeforeDepletion = Math.floor((availableCount + EPSILON) / Math.max(EPSILON, totals.count));
    }
...
function drinkRenewalMode(candidate, itemHrid) {
  const rows = (candidate?.inputItems || []).filter((row) => normalizeHrid(row?.itemHrid) === itemHrid);
...
function evaluateLevelOptions({
  ...
  const normalizedOptimizationMode = normalizeSkillingOptimizationMode(optimizationMode);
  const requiredActiveDrinkHrids = new Set(activeDrinkStateItems(drinkState).map((item) => item.itemHrid));
  const ownedEquipmentHrids = new Set(
    (profile?.equipment || []).map((item) => normalizeHrid(item?.itemHrid)).filter(Boolean),
  );
  ...
  function equipmentLoadoutsForAction(action) {
    ...
    if (!equipmentLoadoutCache.has(cacheKey)) {
      equipmentLoadoutCache.set(
        cacheKey,
        buildSkillingEquipmentLoadouts(profile, skillHrid, skillLevels, data, inventory, reservationSensitiveHrids),
      );
    }
    return equipmentLoadoutCache.get(cacheKey);
  }
  const drinkCache = new Map();
  const mustRankCandidates = collectAlternatives || normalizedOptimizationMode === SKILLING_OPTIMIZATION_MODE_BALANCED;
  ...
  for (const action of actions) {
    ...
    for (const equipmentLoadout of equipmentLoadouts) {
      ...
      if (!drinkCache.has(availableSlotCount)) {
        drinkCache.set(
          availableSlotCount,
          buildDrinkLoadouts(data, actionTypeForSkill(skillHrid), availableSlotCount, activeBuffUniqueHrids),
        );
      }
      for (const drinkLoadout of drinkCache.get(availableSlotCount)) {
        const selectedDrinkHrids = new Set(drinkLoadout.map((drink) => normalizeHrid(drink?.hrid)).filter(Boolean));
        ...
        if (mustRankCandidates) {
          ...
          !best ||
          compareCandidates(candidate, best, normalizedOptimizationMode, null, balancedCostTolerance) < 0
        ) {
          best = candidate;
        }
      }
  ...
  const rankedCandidates = mustRankCandidates
    ? rankCandidates(candidates, normalizedOptimizationMode, balancedLowestCostPerExperience, balancedCostTolerance)
    : [];
  return {
    best: mustRankCandidates ? rankedCandidates[0] || null : best,
    alternatives: collectAlternatives ? rankedCandidates.slice(0, 25) : [],
    missingPriceHrids: Array.from(missingPriceHrids).sort(),
    equipmentLoadoutCount: Array.from(equipmentLoadoutCache.values()).reduce(
      (count, loadouts) => count + loadouts.length,
      0,
    ),
  };
}
...
function sameSegmentStrategy(left, right) {
  return (
    left?.actionHrid === right?.actionHrid &&
...
function mergeRows(leftRows, rightRows, fields) {
  ...
  for (const row of [...(leftRows || []), ...(rightRows || [])]) {
    ...
    if (!itemHrid) continue;
    const key =
      row?.enhancementLevel == null ? itemHrid : `${itemHrid}@${Math.max(0, integer(row.enhancementLevel, 0))}`;
...
function candidateToSegment(candidate, fromLevel, toLevel) {
  const equipmentSignature = candidate.equipment
    .map((item) => `${item.equipmentType}:${item.itemHrid}:${item.enhancementLevel}`)
    .join('|');
...
function mergeSegment(target, addition) {
  const mergedPhases =
    target?.phases || addition?.phases ? [...segmentPhasesForMerge(target), ...segmentPhasesForMerge(addition)] : null;
  const mergedEquipmentStrategies =
    target?.equipmentStrategies || addition?.equipmentStrategies
      ? mergeEquipmentStrategies(equipmentStrategiesForMerge(target), equipmentStrategiesForMerge(addition))
      : null;
  ...
  target.equipmentChanges = finiteNumber(target?.equipmentChanges, 0) + finiteNumber(addition?.equipmentChanges, 0);
...
function summarizeSkillingSegments(segments) {
  ...
  return {
    ...
    materialPurchaseCostPerExperience: totals.experience > 0 ? totals.materialPurchaseCost / totals.experience : 0,
    experiencePerHour: totals.durationHours > 0 ? totals.experience / totals.durationHours : 0,
  };
}

function cloneDrinkState(drinkState) {
  const entries = Object.entries(drinkState?.itemsByHrid || {}).map(([itemHrid, state]) => [itemHrid, { ...state }]);
...
function aggregateLevelActionSegments(segments, actionSortIndex = 0, includePhases = false) {
  ...
  aggregate.equipmentChanges = segments.reduce((sum, segment) => sum + finiteNumber(segment?.equipmentChanges, 0), 0);
...
export function collapseDrinkOnlyLevelPhases(segments) {
  ...
  for (const segment of segments) {
    ...
    const drinkOnlyBoundary =
      sameBaseStrategy &&
      (sameSegmentStrategy(previous, segment) || previous?.drinkSignature !== segment?.drinkSignature);
...
function simulateSkillingLevelAction({
  ...
  const normalizedFallbackOptimizationMode =
    fallbackOptimizationMode == null ? null : normalizeSkillingOptimizationMode(fallbackOptimizationMode);
  ...
  while (simulatedExperience < nextThreshold - EPSILON) {
    const useFallbackOptimization =
      normalizedFallbackOptimizationMode != null && primaryDrinkCyclesUsed >= normalizedPrimaryDrinkCycleLimit;
    ...
    const externalStartWindowSeconds =
      external.nextExpirationAt == null ? null : Math.max(0, (external.nextExpirationAt - simulatedNow) / 1000);
    ...
    const selectedDrinkHrids = new Set(
      (selected.drinks || []).map((drink) => normalizeHrid(drink?.itemHrid)).filter(Boolean),
    );
    ...
    if (repeatedDrinkStrategy) {
      for (const drink of selected.drinks || []) {
        const itemHrid = normalizeHrid(drink?.itemHrid);
        if (itemHrid && finiteNumber(drink?.count, 0) > EPSILON && previousDrinkCycle.seenDrinkHrids.has(itemHrid)) {
    ...
    if (selected.drinkWindowLimited === true && repeatedDrinkStrategy && batchRenewDrinkHrids.length > 0) {
      ...
      if (fullCycleCount > 1) {
        ...
        if (
          ...
          (useFallbackOptimization ||
            normalizedFallbackOptimizationMode == null ||
            !Number.isFinite(normalizedPrimaryDrinkCycleLimit) ||
            candidateDrinkCycleCount(batchEvaluation.best) <= normalizedPrimaryDrinkCycleLimit - primaryDrinkCyclesUsed)
        ) {
    ...
    iteration += 1;
    const progressBasedIterationLimit =
      Number.isFinite(minimumObservedExperiencePerCompletion) && minimumObservedExperiencePerCompletion > EPSILON
        ? Math.ceil(Math.max(0, nextThreshold - currentExperience) / minimumObservedExperiencePerCompletion) + 100
        : 0;
...
export function planSkillingSkill({
  ...
  while (currentExperience < targetExperience - EPSILON) {
    ...
    const availableActions = actionsForSkill.filter(
      (action) => finiteNumber(action?.levelRequirement?.level, 1) <= currentLevel,
    );
    const levelMissingPriceHrids = new Set();
    function simulateAction(action, mode, balancedLowestCostPerExperience = null, simulationOptions = {}) {
    ...
    if (normalizedOptimizationMode === SKILLING_OPTIMIZATION_MODE_BALANCED) {
      ...
      const provisionalBalancedBaseline = [...costSimulations, ...speedSimulations].reduce(
        (minimum, simulation) => Math.min(minimum, finiteNumber(simulation?.alternative?.costPerExperience, Infinity)),
        Infinity,
      );
      const constrainedSimulations = Number.isFinite(provisionalBalancedBaseline)
        ? simulateAvailableActions(SKILLING_OPTIMIZATION_MODE_BALANCED, provisionalBalancedBaseline)
        : [];
      balancedLowestCostPerExperience = [...costSimulations, ...constrainedSimulations, ...speedSimulations].reduce(
        (minimum, simulation) => Math.min(minimum, finiteNumber(simulation?.alternative?.costPerExperience, Infinity)),
        Infinity,
      );
      const costByAction = new Map(costSimulations.map((simulation) => [simulation.actionHrid, simulation]));
      const constrainedByAction = new Map(
        constrainedSimulations.map((simulation) => [simulation.actionHrid, simulation]),
      );
      const speedByAction = new Map(speedSimulations.map((simulation) => [simulation.actionHrid, simulation]));
      const maximumBalancedCostPerExperience =
      ...
      for (const action of availableActions) {
        ...
        if (
          candidateRequiredDurationHours(speedAlternative) >=
          candidateRequiredDurationHours(costAlternative) - EPSILON
        )
          continue;

        const costExcess =
          finiteNumber(costAlternative?.netCost, Infinity) -
          maximumBalancedCostPerExperience * finiteNumber(costAlternative?.gainedExperience, 0);
        const speedExcess =
          finiteNumber(speedAlternative?.netCost, Infinity) -
          maximumBalancedCostPerExperience * finiteNumber(speedAlternative?.gainedExperience, 0);
        if (!(costExcess <= EPSILON && speedExcess > EPSILON)) continue;

        const excessRange = speedExcess - costExcess;
        const estimatedBoundary =
          excessRange > EPSILON ? clamp(-costExcess / excessRange, 0, 1) * speedDrinkCycleCount : 0;
        const lowerBoundary = Math.floor(estimatedBoundary + EPSILON);
        const upperBoundary = Math.ceil(estimatedBoundary - EPSILON);
        const cycleLimits = Array.from(new Set([lowerBoundary, upperBoundary])).filter(
          (limit) => limit > 0 && limit < speedDrinkCycleCount,
        );
        const budgetedSimulations = cycleLimits
          .map((primaryDrinkCycleLimit) =>
            simulateAction(action, SKILLING_OPTIMIZATION_MODE_SPEED, null, {
    ...
    const selectedSimulation = simulations.find((simulation) => simulation.alternative === selectedAlternative);
...
export function buildSkillingOverview(plansBySkill, optimizationMode = SKILLING_OPTIMIZATION_MODE_COST) {
  const normalizedOptimizationMode = normalizeSkillingOptimizationMode(optimizationMode);
  const plans = Object.values(plansBySkill || {}).filter(
    (plan) =>
      plan?.status === 'ok' &&
...
export function resolveSkillingSkillHrids(data, requestedSkillHrids) {
  const supportedSkillHrids = Array.from(
    new Set((data?.skillHrids || []).filter((skillHrid) => typeof skillHrid === 'string' && skillHrid)),
  );
