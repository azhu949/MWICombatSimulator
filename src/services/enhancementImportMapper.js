...
const WISDOM_DRINK_HRIDS = new Set(['/items/wisdom_tea', '/items/wisdom_coffee']);
const DEFAULT_SUPPORT_SLOT_KEYS = ['enhancing_tool', 'hands', 'body', 'legs', 'pouch', 'neck', 'back', 'charm'];
...
function buildEquipmentSlots(rawItems, currentConfig) {
  const supportItems = Array.isArray(enhancementData?.supportEquipment) ? enhancementData.supportEquipment : [];
  ...
  const currentSlots = isPlainObject(currentConfig?.equipmentSlots) ? Object.keys(currentConfig.equipmentSlots) : [];
  const slotKeys = Array.from(
    new Set([...DEFAULT_SUPPORT_SLOT_KEYS, ...discoveredSlots, ...currentSlots].filter(Boolean)),
  );
  const equipmentSlots = Object.fromEntries(
    slotKeys.map((slot) => [
      slot,
      {
        itemHrid: '',
  ...
  for (const rawEntry of toValueList(rawItems)) {
    ...
    const candidate = {
      itemHrid,
      enhancementLevel: clampInteger(entry.enhancementLevel ?? rawEntry?.enhancementLevel, 0, 20, 0),
      isEquipped,
      supportItem,
    };
...
function buildHousePatch(rawHouseRooms) {
  ...
  for (const [roomKey, rawValue] of toKeyedEntries(rawHouseRooms)) {
    const roomHrid = normalizeHrid(rawValue?.houseRoomHrid || rawValue?.roomHrid || rawValue?.hrid || roomKey);
    if (!roomHrid) {
      continue;
    }
...
function buildCommunityPatch(rawCommunityBuffs) {
  ...
  for (const [buffKey, rawValue] of toKeyedEntries(rawCommunityBuffs)) {
    const buffHrid = normalizeHrid(rawValue?.hrid || rawValue?.communityBuffTypeHrid || rawValue?.buffHrid || buffKey);
    if (!buffHrid) {
      continue;
    }
...
function buildAchievementCompletionMap(rawAchievements) {
  ...
  for (const [achievementKey, rawValue] of toKeyedEntries(rawAchievements)) {
    const achievementHrid = normalizeHrid(rawValue?.achievementHrid || rawValue?.hrid || achievementKey);
    if (!achievementHrid) {
      continue;
    }
    completionMap[achievementHrid] =
      rawValue === true || rawValue?.isCompleted === true || rawValue?.completed === true;
  }
...
function isAchievementTierComplete(completionMap, tierHrid) {
  const tierAchievements = Object.values(achievementDetailMap || {}).filter(
    (achievement) => achievement?.tierHrid === tierHrid,
  );
  return (
    tierAchievements.length > 0 && tierAchievements.every((achievement) => completionMap[achievement.hrid] === true)
  );
}
...
export function buildMainSiteEnhancementImport(payload, currentConfig = {}) {
  ...
  if (skills.present) {
    const enhancingSkill = toValueList(skills.value).find(
      (skill) => normalizeHrid(skill?.skillHrid || skill?.hrid) === ENHANCING_SKILL_HRID,
    );
    if (enhancingSkill) {
  ...
  if (achievements.present || achievementBuffs.present) {
    Object.assign(
      configPatch,
      buildAchievementPatch(achievements.value, achievementBuffs.value, achievementBuffs.present),
    );
