...
export function buildQueueBaselineMetricSummary(roundResults = [], medianBlend = QUEUE_MULTI_ROUND_MEDIAN_BLEND_WEIGHT) {
...
export function buildQueueItemCostInsights(
    ...
    const equipmentInspections = typeof inspectQueueEquipmentPricing === "function"
    ...
    const equipmentNetCost = equipmentSaleValue != null && equipmentBuyPrice != null
        ...
        : null;
    const totalUpgradeCostRaw = typeof computeQueueItemUpgradeCost === "function"
...
export function buildQueueRankedRowsFromSampleState({
    ...
    return multiRoundRanking.map((entry) => {
        ...
        const scoringDeltaProfitPct = Math.abs(baselineScoringProfit) > 1e-9
            ? (scoringDeltaProfitPerHour / baselineScoringProfit) * 100
            : 0;
        ...
        const deltaDailyNoRngProfitPct = toFiniteNumber(resolveQueueMetricSummaryDeltaPct(metricSummary, "dailyNoRngProfit"), 0);
