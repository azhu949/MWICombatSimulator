  ...
  for (const monsterHrid of monsters) {
    ...
    for (let index = 0; index < deathsCount; index += 1) {
      for (const dropObject of dropMap.values()) {
        ...
        const amount =
          Math.floor(randomFn() * (dropObject.dropMax - dropObject.dropMin + 1) + dropObject.dropMin) *
          (1 + debuffOnLevelGap) *
...
function createCase(seed) {
  const monsterKeys = Object.keys(combatMonsterDetailMap).filter(
    (key) => Array.isArray(combatMonsterDetailMap[key]?.dropTable) && combatMonsterDetailMap[key].dropTable.length > 0,
  );
