...
describe('skillingPlanner', () => {
  ...
  it('preserves a bonus strategy change when the drink loadout does not change', () => {
    const phases = [displayPhase(), displayPhase({ bonusSignature: 'bonus-from-other-source' })];
  ...
  it('normalizes balanced cost tolerances with a safe default and clamped bounds', () => {
    ...
    expect(normalizeSkillingBalancedCostTolerance(Number.POSITIVE_INFINITY)).toBe(SKILLING_BALANCED_COST_TOLERANCE);
    expect(normalizeSkillingBalancedCostTolerance(Symbol('invalid'))).toBe(SKILLING_BALANCED_COST_TOLERANCE);
  ...
  it('ranks speed candidates by rounded required action duration before throughput and cost', () => {
    ...
    expect(
      [...candidates].sort((left, right) => compareCandidates(left, right, SKILLING_OPTIMIZATION_MODE_COST))[0]
        .actionHrid,
    ).toBe('/actions/brewing/rounded_slower');
    expect(
      [...candidates].sort((left, right) => compareCandidates(left, right, SKILLING_OPTIMIZATION_MODE_SPEED))[0]
        .actionHrid,
    ).toBe('/actions/brewing/fewer_actions');
  });
  ...
  it('keeps wearable inventory loadouts with different reservation identities', () => {
      ...
      equipment: [
        {
          ...
          levelRequirements: [{ skillHrid: SKILL_HRID, level: 1 }],
        ...
        },
        {
          hrid: '/items/fast',
          equipmentType: '/equipment_types/brewing_tool',
          levelRequirements: [{ skillHrid: SKILL_HRID, level: 5 }],
          noncombatStats: { brewingSpeed: 0.2 },
          noncombatEnhancementBonuses: {},
        },
      ],
    };
    ...
    const prunedLoadouts = buildSkillingEquipmentLoadouts(
      profile,
      SKILL_HRID,
      {
        [SKILL_HRID]: { level: 5 },
  ...
  it('selects the cheapest route by default and the shortest rounded route in speed mode', () => {
    ...
    expect(speedPlan.alternatives[0].estimatedLevelDurationHours).toBe(
      speedPlan.alternatives[0].requiredCompletionCount / speedPlan.alternatives[0].actionsPerHour,
    );
    ...
    expect(speedUpgrades.plansBySkill[SKILL_HRID].segments[0].actionHrid).toBe('/actions/brewing/expensive_fast');
  });
  ...
  it('spends the balanced budget on only part of a repeated speed-drink run', () => {
    ...
    const planWithTolerance = (balancedCostTolerance) =>
      planSkillingSkill({
        profile: {
          skills: { [SKILL_HRID]: { level: 1, experience: 0 }, '/skills/total_level': { level: 1 } },
          inventory: {},
  ...
  it('uses the cheapest generated whole-level trajectory as the balanced baseline', () => {
    ...
    expect(result.status).toBe('ok');
    expect(result.segments.every((segment) => segment.actionHrid === '/actions/brewing/stateful')).toBe(true);
  ...
  it('applies the balanced cost tolerance to positive, negative, and zero baselines', () => {
    ...
    function rankedCosts(candidates, baseline, balancedCostTolerance) {
      return [...candidates]
        .sort((left, right) =>
          compareCandidates(left, right, SKILLING_OPTIMIZATION_MODE_BALANCED, baseline, balancedCostTolerance),
        )
    ...
    expect(
      rankedCosts(
        [balancedCandidate('zero-tolerance-baseline', 100, 10), balancedCandidate('zero-tolerance-faster', 100.01, 1)],
        100,
        0,
      ),
    ).toEqual([100, 100.01]);
    ...
    expect(
      [balancedCandidate('fallback-negative-cheapest', -100, 10), balancedCandidate('fallback-negative-faster', -90, 1)]
        .sort((left, right) => compareCandidates(left, right, SKILLING_OPTIMIZATION_MODE_BALANCED))
        .map((candidate) => candidate.costPerExperience),
    ).toEqual([-100, -90]);
  });
  ...
  it('keeps distinct enhancement reservations and consumes the cheaper instance', () => {
    ...
    expect(result.status).toBe('ok');
    expect(result.segments[0].equipment).toEqual([expect.objectContaining({ id: 'enhanced', enhancementLevel: 12 })]);
  ...
  it('protects expensive enhanced bases but allows known cheaper or enhancement-retaining inputs', () => {
    ...
    expect(expensive.inputItems).toEqual([expect.objectContaining({ enhancementLevel: 0, purchaseCount: 1 })]);

    const cheaper = buildCandidate({ enhancedBid: 100_000 });
    expect(cheaper.consumedEquipment).toEqual([expect.objectContaining({ id: 'enhanced-base', enhancementLevel: 7 })]);

    const retained = buildCandidate({ enhancedBid: null, retainAllEnhancement: true });
    expect(retained.consumedEquipment).toEqual([expect.objectContaining({ id: 'enhanced-base', enhancementLevel: 7 })]);
  ...
  it('applies the action floor, xp rounding, artisan, upgrade, and expected drops', () => {
    const candidate = calculateSkillingActionCandidate({
      ...
      equipmentLoadout: emptyLoadout({
        bonuses: { actionSpeed: 10, efficiency: 0, experience: 0.03, essenceFind: 0.2, rareFind: 0.5 },
      }),
  ...
  it('excludes a candidate when inventory is short and no ask exists', () => {
    const candidate = calculateSkillingActionCandidate({
      action: action({
        inputItems: [{ itemHrid: '/items/missing', count: 2 }],
        upgradeItemHrid: '',
        essenceDropTable: [],
        rareDropTable: [],
      }),
      ...
      equipmentLoadout: emptyLoadout(),
      priceTable: priceTable({
        '/items/missing': { ask: -1, bid: 5, vendor: 0 },
        '/items/product': { ask: 1, bid: 1, vendor: 0 },
      }),
    });
  ...
  it('collects gathering and processing Buffs only for gathering skills', () => {
    ...
    const foraging = collectSkillingProfileBonuses(profile, '/action_types/foraging', '/skills/foraging');
    expect(foraging.totals.outputQuantity).toBeCloseTo(0.15);
    expect(foraging.totals.processing).toBeCloseTo(0.2);

    const brewing = collectSkillingProfileBonuses(profile, '/action_types/foraging', SKILL_HRID);
  ...
  it('keeps staged equipment metadata on whole-level alternatives', () => {
    ...
    expect(result.status).toBe('ok');
    expect(result.segments.map((segment) => segment.equipment[0]?.itemHrid)).toEqual([essenceToolHrid, wisdomToolHrid]);
    expect(result.alternatives[0]).toMatchObject({
      completionCount: 3,
      costPerExperience: 3,
    });
    expect(
      result.alternatives[0].equipmentStrategies.map((strategy) => ({
  ...
  it('keeps full-level projections separate from a drink-window initial batch', () => {
    ...
    expect(candidate.estimatedLevelDurationHours).toBe(candidate.requiredCompletionCount / candidate.actionsPerHour);
  ...
  it('does not stack a planned drink with an active Buff from the same unique group', () => {
    ...
    expect(result.segments[1].drinks).toEqual([expect.objectContaining({ itemHrid: '/items/wisdom_tea', count: 1 })]);
  });
  ...
  it('excludes unsupported Processing Tea from foraging candidates', () => {
    ...
    expect(result.status).toBe('ok');
    expect(
      [...result.segments, ...result.alternatives].every((candidate) =>
  ...
  it('reuses remaining drink duration across level boundaries', () => {
    ...
    expect(result.segments[0].drinks).toEqual([expect.objectContaining({ itemHrid: '/items/speed_drink', count: 1 })]);
  });
  ...
  it('charges one new drink when the final action starts before its first expiration', () => {
    ...
    expect(candidate.completionCount).toBe(5);
    expect(candidate.drinks).toEqual([expect.objectContaining({ itemHrid: '/items/wisdom_drink', count: 1 })]);
  ...
  it('only excludes the consumed drink share when an item is also a recipe material', () => {
    ...
    expect(candidate.drinks).toEqual([expect.objectContaining({ itemHrid: '/items/brewing_tea', count: 1 })]);
    expect(candidate.drinkPurchaseCost).toBe(50);
  ...
  it('keeps a cheaper no-drink tail inside the same displayed recipe stage', () => {
    ...
    expect(result.segments).toHaveLength(1);
    expect(result.segments.flatMap((segment) => segment.drinks || [])).toEqual([
    ...
    expect(multiLevelResult.segments[0].phases.reduce((sum, phase) => sum + phase.completionCount, 0)).toBe(
      multiLevelResult.segments[0].completionCount,
    );
    expect(multiLevelResult.segments[0].phases.reduce((sum, phase) => sum + phase.netCost, 0)).toBeCloseTo(
      multiLevelResult.segments[0].netCost,
      10,
    );
  });
  ...
  it('chooses one recipe for the whole level instead of switching only for the tail', () => {
    ...
    expect(result.segments[0].drinks).toEqual([expect.objectContaining({ itemHrid: '/items/wisdom_drink', count: 4 })]);
    expect(result.segments[0].endingDrinkState.itemsByHrid['/items/wisdom_drink'].remainingSeconds).toBe(180);
  ...
  it('keeps the final drink state when adjacent cycles merge into one segment', () => {
    ...
    expect(result.segments[0].endingDrinkState.itemsByHrid['/items/wisdom_drink'].remainingSeconds).toBe(120);
  });
  ...
  it('plans one real level for all six indexed skilling skills', () => {
    ...
    const realPriceTable = Object.fromEntries(
      Object.values(itemDetailIndex).map((item) => [
        item.hrid,
        {
    ...
    expect(
      foragingAction.dropTable.some((drop) =>
  ...
  it('filters requested skills in indexed order and rejects an empty valid selection', () => {
    ...
    const selectedSkillHrids = [supportedSkills[5], '/skills/not_supported', supportedSkills[1], supportedSkills[5]];
    const result = planSkillingUpgrades({
      profile,
  ...
  it('plans a real high-level tailoring level with one whole-level recipe', () => {
    ...
    expect(
      result.alternatives.every(
        (candidate) =>
          candidate.fullLevelCandidate === true && candidate.completionCount === candidate.requiredCompletionCount,
      ),
    ).toBe(true);
  ...
  it('reuses real drink duration across a multi-level production plan', () => {
    ...
    const zeroPriceTable = Object.fromEntries(
      Object.values(itemDetailIndex).map((item) => [
        item.hrid,
        {
          ask: 0,
  ...
  it('ranks only actionable plans with earned experience', () => {
    ...
    expect(overview.map((plan) => plan.skillHrid)).toEqual(['/skills/profitable', '/skills/normal']);
  });
  ...
  it('ranks speed overviews by duration, throughput, cost, and skill HRID', () => {
    const plans = {
      slow: {
        status: 'ok',
        skillHrid: '/skills/slow',
        totalExperience: 100,
        totalDurationHours: 2,
        experiencePerHour: 1_000,
        costPerExperience: -100,
      },
      lowThroughput: {
        status: 'ok',
        skillHrid: '/skills/low_throughput',
        totalExperience: 100,
        totalDurationHours: 1,
        experiencePerHour: 50,
        costPerExperience: -100,
      },
      expensive: {
        status: 'ok',
        skillHrid: '/skills/expensive',
        totalExperience: 100,
        totalDurationHours: 1,
        experiencePerHour: 100,
        costPerExperience: 5,
    ...
    expect(buildSkillingOverview(plans, SKILLING_OPTIMIZATION_MODE_SPEED).map((plan) => plan.skillHrid)).toEqual([
      '/skills/tie_a',
      '/skills/tie_b',
      '/skills/expensive',
  ...
  it('ranks balanced overviews by cost and then duration without a cross-skill tolerance', () => {
    ...
    expect(buildSkillingOverview(plans, SKILLING_OPTIMIZATION_MODE_BALANCED).map((plan) => plan.skillHrid)).toEqual([
