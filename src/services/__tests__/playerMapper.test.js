...
function findFirstNonSpecialAbility() {
  return Object.values(abilityDetailMap).find(
    (ability) => !ability.isSpecialAbility && Array.isArray(ability.defaultCombatTriggers),
  );
}
...
function expectCombatPreviewBreakdownsToReconcile(previewData) {
  for (const breakdown of Object.values(previewData.statBreakdowns)) {
    const sourceTotal = breakdown.sources.reduce((sum, source) => sum + source.deltaValue, 0);
    expect(breakdown.baseValue + sourceTotal + breakdown.reconciliationDelta).toBeCloseTo(breakdown.finalValue, 10);
  }
}
...
describe('playerMapper', () => {
  ...
  it('maps selected player config to simulation players', () => {
    ...
    const weapon =
      findFirstEquipmentByType('/equipment_types/main_hand') || findFirstEquipmentByType('/equipment_types/two_hand');
  ...
  it('applies housing permanent buffs only in combat preview builds', () => {
    ...
    expect(simulationPlayer.combatDetails.combatStats.hpRegenPer10).toBeLessThan(
      previewPlayer.combatDetails.combatStats.hpRegenPer10,
    );
    expect(simulationPlayer.combatDetails.combatStats.castSpeed).toBeLessThan(
      previewPlayer.combatDetails.combatStats.castSpeed,
    );
    expect(simulationPlayer.combatDetails.combatStats.combatExperience).toBeLessThan(
      previewPlayer.combatDetails.combatStats.combatExperience,
    );
    expect(simulationPlayer.combatDetails.combatStats.combatRareFind).toBeLessThan(
      previewPlayer.combatDetails.combatStats.combatRareFind,
    );
  ...
  it('attributes each active combat shrine to only the stats it changes', () => {
    ...
    for (const shrineCase of shrineCases) {
      ...
      const guildSources = previewData.highlightSources.filter((source) => source.sourceType === 'guild_buff');
  ...
  it('applies combat scrolls to ordinary previews and ignores them in Labyrinth', () => {
    ...
    const zonePreview = buildCombatPreviewData(
      player,
      {
    ...
    const disabledPreview = buildCombatPreviewData(
      player,
      {
    ...
    const labyrinthPreview = buildCombatPreviewData(
      player,
      {
        combatScrollsEnabled: true,
      },
    ...
    expect(
      zonePreview.highlightSources
        .filter((source) => source.sourceType === 'combat_scroll')
        .map((source) => source.sourceHrid),
    ).toEqual(['/items/seal_of_damage', '/items/seal_of_wisdom']);
  ...
  it('restores the complete preview state after deriving all scroll highlights', () => {
    ...
    player.combatScrolls = Object.fromEntries(itemHrids.map((itemHrid) => [itemHrid, { quantity: null }]));
    player.abilities[0] = { abilityHrid: '/abilities/speed_aura', level: 1 };

    const preview = buildCombatPreviewData(
      player,
      {
        combatScrollsEnabled: true,
      },
    ...
    expect(combatScrollOptions.every((option) => preview.player.combatBuffs[option.buff.uniqueHrid])).toBe(true);
    ...
    const speedAuraSource = preview.highlightSources.find((source) => source.sourceHrid === '/abilities/speed_aura');
    const speedAuraAttackInterval = speedAuraSource?.changedStats.find((stat) => stat.key === 'attackIntervalSeconds');
    ...
    expect(speedAuraAttackInterval.finalValue - speedAuraAttackInterval.deltaValue).toBeCloseTo(
      fullScrollAttackInterval,
      6,
    );
  ...
  it('returns triggered final stats with reconciled shrine, scroll, drink, and ability sources', () => {
    ...
    const previewData = buildCombatPreviewData(
      player,
      {
        combatScrollsEnabled: true,
      },
    ...
    expect(previewData.statBreakdowns.attackIntervalSeconds.finalValue).toBeCloseTo(finalAttackInterval / 1e9, 10);
    expectCombatPreviewBreakdownsToReconcile(previewData);
  });
  ...
  it('builds drink preview cards with the consumed drink state and effective cooldown', () => {
    ...
    expect(previewData.player.combatDetails.combatStats.castSpeed).toBe(
      buildPlayersForCombatPreview([player])[0].combatDetails.combatStats.castSpeed,
    );
  ...
  it('skips drink highlights and downstream buff inflation when a custom drink trigger is inactive', () => {
    ...
    expect(previewData.finalPlayer.combatDetails.magicMaxDamage).toBeCloseTo(
      previewData.player.combatDetails.magicMaxDamage,
      10,
    );
    expectCombatPreviewBreakdownsToReconcile(previewData);
  });
  ...
  it('surfaces equipped buff abilities as conditional combat preview highlights', () => {
    ...
    const elementalAffinitySource = abilitySources.find(
      (source) => source.sourceHrid === '/abilities/elemental_affinity',
    );
  ...
  it('surfaces enemy-triggered buff abilities when a combat preview enemy is present', () => {
    ...
    const previewData = buildCombatPreviewData(player);
    const manaSpringSource = previewData.highlightSources.find(
      (source) => source.sourceHrid === '/abilities/mana_spring',
    );
  ...
  it('keeps guaranteed pierce chains when evaluating all-enemies missing-hp triggers', () => {
    ...
    player.abilities[1] = { abilityHrid: '/abilities/elemental_affinity', level: 80 };
    const initialManapoints = buildPlayersForCombatPreview([player], null, previewContext)[0].combatDetails
      .currentManapoints;
  ...
  it('surfaces retaliation-only buff abilities as combat preview highlights', () => {
    ...
    const previewData = buildCombatPreviewData(player);
    const retributionSource = previewData.highlightSources.find(
      (source) => source.sourceHrid === '/abilities/retribution',
    );
