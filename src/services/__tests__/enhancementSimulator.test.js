...
describe('enhancementSimulator', () => {
  ...
  it('resolves data-driven equipment and buff bonuses', () => {
    expect(
      resolveEnhancedStat(
        enhancementData.supportEquipment[0],
        'enhancingSuccess',
        2,
        enhancementData.totalBonusMultipliers,
      ),
    ).toBeCloseTo(0.0121, 10);

    const bonuses = resolveEnhancementBonuses(
      baseConfig({
  ...
  it('applies the low-skill penalty and clamps final success chances', () => {
    ...
    const cappedModel = buildEnhancementTransitionModel(
      baseConfig({
  ...
  it('supports arbitrary starts, blessed target overshoot, and +20', () => {
    ...
    const blessed = analyzeEnhancementStrategy(
      baseConfig({ targetLevel: 2, blessedEnabled: true }),
      enhancementData,
      pricing,
    );
    ...
    const plusTwenty = analyzeEnhancementStrategy(
      baseConfig({ targetLevel: 20, protectAt: 2 }),
      enhancementData,
      pricing,
    );
  ...
  it('accepts compact ask/bid aliases in price overrides', () => {
    ...
    expect(result.materialPriceDetails.find((entry) => entry.itemHrid === '/items/dust')).toMatchObject({
      unitPrice: 99,
  ...
  it('keeps side-specific overrides isolated until the requested market side is missing', () => {
    ...
    expect(
      resolveEnhancementPrice(pricing, '/items/dust', {
        mode: 'ask',
        overrides,
      }),
    ...
    expect(
      resolveEnhancementPrice(missingAskPricing, '/items/dust', {
        mode: 'ask',
        overrides,
      }),
    ).toMatchObject({ price: 7, source: 'override', priceSide: 'bid' });
  ...
  it('prices selected teas from action time, duration, and drink concentration', () => {
    ...
    for (const itemHrid of ['/items/enhancing_tea', '/items/blessed_tea', '/items/wisdom_tea']) {
      expect(result.perActionMaterials[itemHrid]).toBeCloseTo(expectedCountPerAction, 12);
      expect(result.expectedMaterialCounts[itemHrid]).toBeCloseTo(result.expectedActions * expectedCountPerAction, 10);
    }
  ...
  it('applies the special 18% market tax to Bag of 10 Cowbells other-loot bids', () => {
    ...
    const estimate = estimateEnhancementAcquisitionValue(nonTradableCloakHrid, specialLootData, specialLootPricing);
  ...
  it('uses vendor liquidation only after a missing bid and requires asks for acquisition inputs', () => {
    ...
    expect(vendorEstimate.otherLootDetails.find((drop) => drop.itemHrid === '/items/other_loot')).toMatchObject({
      unitPrice: 1_000_000,
      priceSource: 'vendor',
    });
    ...
    expect(
      estimateEnhancementAcquisitionValue(nonTradableCloakHrid, acquisitionEnhancementData, missingAskPricing),
    ).toMatchObject({
      available: false,
  ...
  it('clamps acquisition estimates to vendor value and chooses the cheapest complete route', () => {
    ...
    expect(
      estimateEnhancementAcquisitionValue(nonTradableCloakHrid, acquisitionEnhancementData, highLootPricing),
  ...
  it('switches to moment-Gamma when estimated Monte Carlo work is high', () => {
    const strategy = analyzeEnhancementStrategy(
      baseConfig({ targetLevel: 20, protectAt: 2 }),
      enhancementData,
      pricing,
    );
