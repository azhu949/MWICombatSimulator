...
import { buildMainSiteEnhancementImport, mapMainSiteEnhancementConfig } from '../enhancementImportMapper.js';
...
describe('enhancementImportMapper', () => {
  it('maps current main-site character bonuses into enhancement configuration', () => {
    const payload = {
      ...
      characterSkills: [{ skillHrid: '/skills/enhancing', level: 137 }],
      characterItems: [
        {
          hash: 'tool',
    ...
    expect(result.importedSections).toEqual(['skill', 'equipment', 'drinks', 'housing', 'community', 'achievements']);
  ...
  it('falls back to completed achievement tiers when the server buff map is unavailable', () => {
    ...
    const incompleteChampion = tierAchievements.filter(
      (entry) => entry.achievementHrid !== '/achievements/enhance_level_90_to_10',
    );
