...
import { analyzeEnhancementRisk, analyzeEnhancementStrategy, runMonteCarloTrials } from '../enhancementSimulator.js';
...
describe('enhancementWorker runtime', () => {
  ...
  it('prevents a superseded run from publishing a late result', async () => {
    ...
    expect(messages.filter((message) => message.type === 'enhancement_result' && message.runId === 'old')).toHaveLength(
      0,
    );
    expect(messages.filter((message) => message.type === 'enhancement_result' && message.runId === 'new')).toHaveLength(
      1,
    );
  });
  ...
  it('falls back to exact moments and Gamma at the transition hard limit', async () => {
    ...
    expect(result).toEqual(
      analyzeEnhancementRisk(strategy, {
        sampleCount: 100,
        hardTransitionLimit: 1,
        loadThreshold: Number.MAX_SAFE_INTEGER,
      }),
