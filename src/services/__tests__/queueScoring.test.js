...
describe("queueScoring", () => {
    ...
    it("summarizes non-finite values as zero while ignoring missing delta percentages", () => {
        const summary = summarizeMetric([Number.NaN, Number.POSITIVE_INFINITY, Number.NEGATIVE_INFINITY], [
            undefined,
            Number.NaN,
        ], 0.5);
    ...
    it("resolves robust delta percentages before legacy mean aliases", () => {
        expect(resolveQueueMetricSummaryDeltaPct({
            dps: {
    ...
    it("builds cost insights using injected equipment and upgrade-cost dependencies", () => {
        ...
        expect(computeQueueItemUpgradeCost).toHaveBeenCalledWith(
            baselineSnapshot,
            targetSnapshot,
            pricingState,
            {
    ...
    it("keeps visible profit deltas on the robust metric basis while preserving scoring deltas", () => {
        ...
        const createRawRun = (round, dailyNoRngProfit) => ({
            ...
            deltas: {
                dps: { pct: 0 },
                dailyNoRngProfit: { pct: ((dailyNoRngProfit - baselineMetrics.dailyNoRngProfit) / baselineMetrics.dailyNoRngProfit) * 100 },
                xpPerHour: { pct: 0 },
                killsPerHour: { pct: 0 },
            },
        });
        ...
        const rows = buildQueueRankedRowsFromSampleState({
            ...
            rawRuns: [
                ...Array.from({ length: 20 }, (_, index) => createRawRun(index + 1, 2400)),
                createRawRun(21, 24000),
            ],
            queueSettings: {
                rounds: 21,
                baselineRounds: 1,
