export function createMainSiteShareProfileFixture(options = {}) {
  ...
  const totalLevel = Object.values(normalizedSkills).reduce(
    (sum, value) => sum + Math.max(1, Math.floor(Number(value || 1))),
    0,
  );
    ...
    characterSkills: [
      {
        ...
        level: normalizedSkills.stamina,
        experience: Number(skillExperience.stamina ?? 0),
      },
      {
        skillHrid: '/skills/intelligence',
        level: normalizedSkills.intelligence,
        experience: Number(skillExperience.intelligence ?? 0),
      },
      ...
      { skillHrid: '/skills/melee', level: normalizedSkills.melee, experience: Number(skillExperience.melee ?? 0) },
      {
        ...
        level: normalizedSkills.defense,
        experience: Number(skillExperience.defense ?? 0),
      },
...
export function createMainSiteCurrentCharacterFixture(options = {}) {
  const {
    characterId = 101,
    characterName = 'Main Site Current Hero',
    skills = {},
    skillExperience = {},
    characterItems = [],
    combatAbilities = [],
  ...
  return {
    ...
    actionTypeFoodSlotsMap: { ...actionTypeFoodSlotsMap },
    actionTypeDrinkSlotsMap: { ...actionTypeDrinkSlotsMap },
    consumableCombatTriggersMap: { ...consumableCombatTriggersMap },
    abilityCombatTriggersMap: { ...abilityCombatTriggersMap },
    characterHouseRoomMap: { ...characterHouseRoomMap },
