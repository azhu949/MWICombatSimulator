...
function findFirstAbility() {
    const ability = Object.values(abilityDetailIndex || {}).find((entry) => (
        String(entry?.hrid || "")
...
describe("queueVariants", () => {
    it("splits multi-change queue candidates into one-change variants", () => {
        ...
        expect(summary.changes.map((change) => change.kind)).toEqual([
            "level",
            "equipment",
            "house_room",
        ]);
    ...
    it("serializes queue change templates without before fields and rejects trigger-only templates", () => {
        ...
        expect(normalizeEquipmentSetQueueChanges({
            items: [{
                name: "",
                targets: [
        ...
        expect(queueStateHasUnsupportedEquipmentSetQueueChanges({
    ...
    it("rebuilds queue items from change templates with deterministic ids and timestamps", () => {
        ...
        expect(builtItems[0].changeDetails.map((change) => change.kind)).toEqual(expect.arrayContaining([
            "level",
            "equipment",
            "food",
            "ability",
