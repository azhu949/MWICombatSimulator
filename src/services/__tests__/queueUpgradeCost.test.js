...
function findEquipmentForSlot(slotKey = "head") {
    ...
    const item = Object.values(itemDetailIndex || {}).find((entry) => (
        entry?.categoryHrid === "/item_categories/equipment"
        && String(entry?.equipmentDetail?.type || "") === typeHrid
    ));
    return item?.hrid ?? "";
}

function findAbilityBookInfo() {
    const entry = Object.entries(abilityBookInfoByAbilityHrid || {}).find(([, info]) => (
        String(info?.itemHrid || "")
        && Number(info?.xpPerBook || 0) > 0
    ));
    if (!entry) {
        return null;
    }
...
function findHouseRoomWithLevel(level = 1) {
    return Object.values(houseRoomDetailIndex || {}).find((entry) => (
        String(entry?.hrid || "")
...
describe("queueUpgradeCost", () => {
    ...
    it("computes ability upgrade cost from reference globals and book prices", () => {
        ...
        try {
            ...
            expect(computeDefaultAbilityUpgradeCost({
                abilityHrid: abilityBookInfo.abilityHrid,
                level: 1,
            }, 3, pricingState)).toBe(Math.ceil(250 / abilityBookInfo.xpPerBook) * 7);
        } finally {
    ...
    it("combines equipment, custom ability, and house-room costs for queue items", () => {
        ...
        const housePreview = buildHouseRoomUpgradeCostPreview(
            baseline.houseRooms,
            target.houseRooms,
            pricingState
        );
        ...
        expect(computeQueueItemUpgradeCost(
            baseline,
            target,
            pricingState,
            {
    ...
    it("resolves recent trade averages from the cached enhancement quote table", () => {
        ...
        expect(resolveRecentTradeAverage(buildPricingState({
            marketTimestamp: 777,
            enhancementQuotesByItem: {
                [equipmentHrid]: {
                    "5": {
