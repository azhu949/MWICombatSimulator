import { actionDetailIndex, monsterDetailIndex } from '../shared/gameDataIndex.js';
import {
...
export function normalizeZoneSelection(selectedHrids, allHrids) {
  ...
  return Array.from(new Set(selected.map((value) => String(value || '')).filter((value) => allSet.has(value))));
}
...
export function buildZoneTargetsByScope(runScope, selectedZoneHrids = []) {
  ...
  if (runScope === RUN_SCOPE_ALL_GROUP_ZONES) {
    actions = actions.filter(
      (action) => Number(action?.combatZoneInfo?.fightInfo?.randomSpawnInfo?.maxSpawnCount ?? 0) > 1,
    );
  }
...
export function buildSingleSimulationPayload(
  playersToSim,
  simulationSettings,
  activeLabyrinthCrates = [],
  options = {},
) {
...
export function computeQueueMetrics(simResult, preferredPlayerId, pricingOptions = {}) {
  ...
  const totalExperience = Object.values(simResult?.experienceGained?.[playerHrid] ?? {}).reduce(
