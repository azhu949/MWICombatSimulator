...
describe("Hero party trigger combat preview", () => {
    it("evaluates hero all-allies triggers against the selected party", () => {
        ...
        expect(soloPreview.finalPlayer.combatBuffs["/buff_uniques/speed_aura_attack_speed"])
            .toBeFalsy();
        ...
        expect(partyPreview.finalPlayer.combatBuffs["/buff_uniques/speed_aura_attack_speed"])
            .toBeTruthy();
        expect(partyPreview.highlightSources.some((source) => (
            source.sourceKey === "ability-1-/abilities/speed_aura"
        ))).toBe(true);
    });
});
