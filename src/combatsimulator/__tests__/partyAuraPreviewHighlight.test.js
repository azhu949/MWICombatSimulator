...
describe("Party aura combat preview highlight attribution", () => {
    it("drops the hero's own aura source when a teammate's stronger aura owns the buff", () => {
        ...
        expect(
            preview.highlightSources.some((source) => source.sourceKey === "ability-1-/abilities/speed_aura"),
        ).toBe(false);
    ...
    it("keeps the hero's own aura source when it beats every teammate aura", () => {
        ...
        expect(
            withParty.highlightSources.some((source) => source.sourceKey === "ability-1-/abilities/speed_aura"),
        ).toBe(true);
        // The weaker teammate cast still contributes no source.
        expect(
            withParty.highlightSources.some((source) => source.sourceKey?.startsWith("teammate-aura-")),
        ).toBe(false);
    });
});
