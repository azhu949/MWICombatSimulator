...
const EXPECTED_LEVEL_VALUES = {
  ...
  '/guild_buffs/rarity_combat': [
    { typeHrid: '/buff_types/rare_find', field: 'flatBoost', level1: 0.015, level20: 0.3 },
  ],
...
describe('guild shrine combat buffs', () => {
  ...
  it('applies all five shrine effects to derived combat stats', () => {
    ...
    const maxed = activatePermanentBuffs(
      buildPlayer(Object.fromEntries(combatGuildBuffHrids.map((hrid) => [hrid, 20]))),
    );

    expect(maxed.combatDetails.smashMaxDamage / baseline.combatDetails.smashMaxDamage).toBeCloseTo(1.06, 8);
    expect(maxed.combatDetails.combatStats.attackInterval).toBeLessThan(
      baseline.combatDetails.combatStats.attackInterval,
    );
    expect(maxed.combatDetails.combatStats.castSpeed - baseline.combatDetails.combatStats.castSpeed).toBeCloseTo(
      0.08,
      8,
    );
    ...
    expect(
      maxed.combatDetails.combatStats.combatRareFind - baseline.combatDetails.combatStats.combatRareFind,
    ).toBeCloseTo(0.3, 8);
    expect(
      maxed.combatDetails.combatStats.combatExperience - baseline.combatDetails.combatStats.combatExperience,
    ).toBeCloseTo(0.1, 8);
  });
  ...
  it('preserves shrine buffs through the worker DTO roundtrip', () => {
    ...
    expect(roundtrip.guildBuffs.map((guildBuff) => [guildBuff.hrid, guildBuff.level])).toEqual(
      direct.guildBuffs.map((guildBuff) => [guildBuff.hrid, guildBuff.level]),
    );
    ...
    expect(roundtrip.combatDetails.combatStats.combatExperience).toBe(
      direct.combatDetails.combatStats.combatExperience,
    );
  });
});
