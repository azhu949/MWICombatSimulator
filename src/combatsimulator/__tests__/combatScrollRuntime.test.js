...
function totalExperience(simulator, playerHrid = 'player1') {
  return Object.values(simulator.simResult.experienceGained[playerHrid] || {}).reduce(
...
describe('CombatSimulator combat scroll runtime', () => {
  it('opens at t=0 and applies an independent Buff instance per player', () => {
    ...
    const { simulator, players } = prepareSimulator(
      [{ [itemHrid]: { quantity: null } }, { [itemHrid]: { quantity: null } }],
      MINUTE_30,
    );
    ...
    expect(players[0].combatBuffs['/buff_uniques/personal_attack_speed']).not.toBe(
      players[1].combatBuffs['/buff_uniques/personal_attack_speed'],
    );
  ...
  it('removes a scroll source even when another stronger source is active', () => {
    const itemHrid = '/items/seal_of_attack_speed';
    const { simulator, players } = prepareSimulator([{ [itemHrid]: { quantity: 1 } }], MINUTE_30);
  ...
  it.each([
    ...
    const { simulator } = prepareSimulator([{ [itemHrid]: { quantity: null } }], limit);
  ...
  it('only performs a full scroll sync when the next renewal is due', async () => {
    const itemHrid = '/items/seal_of_damage';
    const { simulator } = prepareSimulator([{ [itemHrid]: { quantity: null } }], MINUTE_30 * 2);
  ...
  it('renews finite inventory and stops after the configured quantity', () => {
    ...
    const { simulator } = prepareSimulator([{ [itemHrid]: { quantity: 2 } }], limit);
  ...
  it("keeps each player's inventory and effects independent", () => {
    ...
    const { simulator, players } = prepareSimulator(
      [{ [damage]: { quantity: 1 } }, { [wisdom]: { quantity: 2 } }],
      MINUTE_30 * 2,
    );
  ...
  it('opens 48 unlimited scrolls in a 24-hour simulation', () => {
    ...
    const { simulator } = prepareSimulator([{ [itemHrid]: { quantity: null } }], DAY_NS);
  ...
  it('retains configured rows and marks the result when scroll effects are paused', () => {
    const itemHrid = '/items/seal_of_damage';
    const { simulator, players } = prepareSimulator([{ [itemHrid]: { quantity: 3 } }], MINUTE_30, {
  ...
  it('can finalize an active window without re-attaching a missing buff', () => {
    const itemHrid = '/items/seal_of_damage';
    const { simulator, players } = prepareSimulator([{ [itemHrid]: { quantity: null } }], MINUTE_30);
  ...
  it('keeps player regeneration stable across repeated scroll refreshes', () => {
    const itemHrid = '/items/seal_of_attack_speed';
    const { simulator, players } = prepareSimulator([{ [itemHrid]: { quantity: null } }], MINUTE_30 * 3 + ONE_NS);
    const player = players[0];
  ...
  it('keeps the recorded death time when encounter cleanup is delayed', () => {
    ...
    expect(firstSnapshot.experienceRate).toBeCloseTo(1 + 5_000_000_000 / firstEnemy.enrageTime, 10);
    ...
    const firstGain = Object.values(firstSnapshot.gainsByPlayer.player1).reduce((sum, value) => sum + value, 0);
    const secondGain = Object.values(secondSnapshot.gainsByPlayer.player1).reduce((sum, value) => sum + value, 0);
    expect(firstGain / firstSnapshot.experienceRate).toBeGreaterThan(secondGain / secondSnapshot.experienceRate);
  });
  ...
  it.each([
    ...
    const { simulator } = prepareSimulator([{}], MINUTE_30);
    const enemies = [createTestEnemy('/monsters/invalid_enrage', 0), createTestEnemy('/monsters/invalid_enrage', 0)];
  ...
  it('matches the legacy aggregate experience for an unbuffed cleared encounter', () => {
    const { simulator } = prepareSimulator([{}], MINUTE_30);
    const enemies = [createTestEnemy('/monsters/one', 0, 100), createTestEnemy('/monsters/two', 0, 150)];
  ...
  it('integrates scroll windows with a real zone simulation and kill-time buckets', async () => {
    ...
    expect(simulator.eventQueue.minHeap.toArray().filter((event) => event.type === 'scrollRenewal')).toHaveLength(0);
  });
});
