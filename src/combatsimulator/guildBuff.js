...
import { getGuildBuffMaxLevel, guildBuffDetailIndex } from '../shared/guildBuffs.js';
...
class GuildBuff {
  constructor(hrid, level) {
    ...
    this.hrid = hrid;
    this.level = Math.min(Math.max(0, Math.floor(Number(level) || 0)), getGuildBuffMaxLevel(hrid));
    this.buffs = this.level > 0 ? (detail.buffs || []).map((buff) => new Buff(buff, this.level)) : [];
  }
