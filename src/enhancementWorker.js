...
function normalizeRun(payload) {
  ...
  const loadThreshold = Math.max(0, toFiniteNumber(options.loadThreshold, MONTE_CARLO_LOAD_THRESHOLD));
...
export function createEnhancementWorkerRuntime(runtimeOptions = {}) {
  const post = typeof runtimeOptions.postMessage === 'function' ? runtimeOptions.postMessage : () => {};
  const yieldTask = typeof runtimeOptions.yieldTask === 'function' ? runtimeOptions.yieldTask : defaultYieldTask;
  let activeRun = null;
  ...
  async function executeRun(payload) {
    ...
    try {
      ...
      const compactTrials = includeTrials ? null : createMonteCarloTrialAccumulator(run.strategy, run.options);
      let completed = 0;
      let transitions = 0;
