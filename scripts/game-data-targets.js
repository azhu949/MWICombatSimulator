...
function getTrackedGameDataFileNames() {
  ...
  const fileNames = fs
    .readdirSync(TRACKED_GAME_DATA_DIR, { withFileTypes: true })
  ...
  return Array.from(new Set([...DEFAULT_TRACKED_GAME_DATA_FILES, ...fileNames])).sort((left, right) =>
...
function getMissingTargetKeys(clientData, targetMapFiles = TARGET_MAP_FILES) {
  ...
  return Object.keys(targetMapFiles).filter(
    (key) => !Object.prototype.hasOwnProperty.call(clientData, key) || typeof clientData[key] === 'undefined',
  );
}
