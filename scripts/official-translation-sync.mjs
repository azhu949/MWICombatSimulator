...
function getObjectProperty(objectNode, name) {
  ...
  return (
    objectNode.properties.find((property) => property?.type === 'Property' && getPropertyName(property) === name) ||
    null
  );
}
...
function findHomeMainAssetPath(homeSource) {
  const match = String(homeSource || '').match(/<script[^>]+src=["'](\/static\/js\/main\.[^"']+\.chunk\.js)["']/i);
...
function findHomeChunkAssetPath(homeSource, chunkId) {
  ...
  return hashMatch?.[1] ? `/static/js/${chunkId}.${hashMatch[1]}.chunk.js` : '';
}
  ...
  try {
    ...
    for (const entry of entries) {
      try {
        ...
        const canRetryWindowsReplacement = entry.hadOriginal && ['EACCES', 'EEXIST', 'EPERM'].includes(error?.code);
