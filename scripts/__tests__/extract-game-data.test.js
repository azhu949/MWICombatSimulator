...
describe('extract-game-data CLI', () => {
  it('rejects payloads that omit the required enhancement success-rate table', () => {
    ...
    fs.writeFileSync(path.join(outputDir, 'enhancementLevelSuccessRateTable.json'), '[0.99]\n', 'utf8');
    fs.writeFileSync(inputPath, `${JSON.stringify(incompleteClientData, null, 2)}\n`, 'utf8');

    expect(() =>
      execFileSync(process.execPath, [SCRIPT_PATH, '--input', inputPath, '--output', outputDir], {
        cwd: path.resolve('.'),
        stdio: 'pipe',
      }),
    ).toThrow();
  ...
  it.each(['guildBuffDetailMap', 'guildShrineDetailMap'])('rejects payloads that omit required %s', (requiredKey) => {
    const tempDir = createTempDir();
    const inputPath = path.join(tempDir, 'initClientData.json');
    const outputDir = path.join(tempDir, 'maps');
    const incompleteClientData = { ...BASE_CLIENT_DATA };
  ...
  it.each(['guildBuffDetailMap', 'guildShrineDetailMap'])(
    'rejects payloads that contain empty required %s',
    (requiredKey) => {
      const tempDir = createTempDir();
      const inputPath = path.join(tempDir, 'initClientData.json');
      const outputDir = path.join(tempDir, 'maps');
  ...
  it('writes the full resolved clientData JSON when --all is enabled', () => {
    ...
    execFileSync(
      process.execPath,
      [SCRIPT_PATH, '--input', inputPath, '--output', outputDir, '--all', '--all-output', allOutputPath],
      {
        cwd: path.resolve('.'),
        stdio: 'pipe',
      },
    ...
    expect(JSON.parse(fs.readFileSync(path.join(outputDir, 'enhancementLevelSuccessRateTable.json'), 'utf8'))).toEqual(
      BASE_CLIENT_DATA.enhancementLevelSuccessRateTable,
    );
    expect(JSON.parse(fs.readFileSync(path.join(outputDir, 'personalBuffTypeDetailMap.json'), 'utf8'))).toEqual(
      BASE_CLIENT_DATA.personalBuffTypeDetailMap,
    );
  ...
  it('resets optional community buff data to an empty object when the payload no longer includes it', () => {
    ...
    const output = execFileSync(process.execPath, [SCRIPT_PATH, '--input', inputPath, '--output', outputDir], {
      cwd: path.resolve('.'),
      stdio: 'pipe',
    }).toString('utf8');
    ...
    expect(output).toContain(
      'Reset 1 optional tracked game-data file to fallback because the payload did not include it:',
    );
  ...
  it('resets runtime label detail maps to empty objects when the payload no longer includes them', () => {
    ...
    expect(output).toContain(
      'Reset 3 optional tracked game-data file to fallback because the payload did not include it:',
    );
