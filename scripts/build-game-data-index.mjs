...
const ENHANCEMENT_ACHIEVEMENT_BUFF_TYPE_HRIDS = new Set(['/buff_types/enhancing_success', '/buff_types/wisdom']);
...
const SKILLING_ACTION_FUNCTION_SET = new Set(['/action_functions/gathering', '/action_functions/production']);
const SKILLING_SKILL_HRIDS = Object.freeze(
  SKILLING_ACTION_TYPE_HRIDS.map((hrid) => hrid.replace('/action_types/', '/skills/')),
);
...
function createItemIndex(itemDetailMap, equipmentTypeDetailMap) {
  ...
  for (const item of Object.values(itemDetailMap || {})) {
    ...
    itemDetailIndex[hrid] = {
      ...
      consumableDetail: {
        ...
        manapointRestore,
        recoveryDuration,
        defaultCombatTriggers: Array.isArray(item?.consumableDetail?.defaultCombatTriggers)
          ? item.consumableDetail.defaultCombatTriggers
          : [],
      },
      defaultCombatTriggers: Array.isArray(item?.consumableDetail?.defaultCombatTriggers)
        ? item.consumableDetail.defaultCombatTriggers
  ...
  foodOptions.sort(
    (left, right) =>
      resolveFoodConsumableSortGroup(left) - resolveFoodConsumableSortGroup(right) || sortByNameThenLevel(left, right),
  );
...
function createEnhancementAcquisitionIndex({ actionDetailMap, enhanceableItems, itemDetailMap, openableLootDropMap }) {
  ...
  for (const [containerHrid, rawDrops] of Object.entries(openableLootDropMap || {})) {
    ...
    const targetHrids = Array.from(
      new Set(drops.map((drop) => drop.itemHrid).filter((itemHrid) => nonTradableEnhanceableHrids.has(itemHrid))),
    );
...
function createEnhancementIndex({
  ...
  const catalogItemsForHrids = (hrids) =>
    Array.from(hrids)
  ...
  const globalExperienceBuff =
    roomsWithGlobalExperience
...
function createSkillingIndex({ actionDetailMap, enhancementLevelTotalBonusMultiplierTable, itemDetailMap }) {
  ...
  for (const item of Object.values(itemDetailMap || {})) {
    ...
    if (
      ...
      (drinkSlots > 0 || hasSkillingSupportStat(noncombatStats) || hasSkillingSupportStat(noncombatEnhancementBonuses))
    ) {
      equipment.push({
        ...createEnhancementCatalogItem(item),
    ...
    const usableInActionTypeMap = Object.fromEntries(
      SKILLING_ACTION_TYPE_HRIDS.filter(
        (actionTypeHrid) => item?.consumableDetail?.usableInActionTypeMap?.[actionTypeHrid] === true,
