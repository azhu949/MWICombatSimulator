...
`scripts/mwi-main-site-import.user.js` 是一个单文件 Tampermonkey userscript，用来通过 **单个导入按钮** 把 **Milky Way Idle 主站当前角色** 或 **已手动打开并缓存过资料的当前队伍成员** 导入到 **MWI Combat Simulator**。
...
- 从已打开的主站标签页读取当前角色状态，并在识别到队伍时优先复用你手动打开过的队友资料缓存

它不会直接调用游戏 HTTP API，也不会自动向主站发送打开玩家资料的请求。

## 主要功能

- 单按钮导入当前角色：默认导入主站当前角色到模拟器当前激活玩家槽位
- 队伍部分导入：识别到队伍时，会按顺序导入你已手动打开并缓存过资料的成员到 `Player 1..N` 槽位（最多 5），缺失成员会跳过并提示
...
- 若部分队友资料尚未手动打开过，会跳过这些成员，并在状态提示中列出需要先打开资料的名字
...
请先确认主站当前角色页面状态是否完整加载；如果主站刚切角色、刚登录或连接刚恢复，建议刷新主站后重试。若是队伍导入缺人，请先在游戏中手动打开对应队友资料一次。
...
- 队伍成员资料不会自动抓取；只有你手动打开过并缓存成功的成员才会被导入
- 不支持账号下多角色批量映射
- 必须至少保留一个已登录的主站标签页
- 脚本依赖主站当前角色初始化数据与手动打开资料产生的 `profile_shared`；如果主站未来改动这些链路，脚本需要跟进更新

## 隐私与安全

- 不会修改主站角色数据
- 不会发起额外的游戏 API 写请求
- 不会自动发送 `view_profile`，也不会自动关闭玩家资料弹窗
- 只读取浏览器里当前主站页面已经可见的当前角色状态，以及你手动打开资料后产生的玩家资料数据
