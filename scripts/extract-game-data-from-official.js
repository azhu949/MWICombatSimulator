...
function usage() {
  ...
  console.log(
    '                        Missing tracked files are skipped with a warning; optional fallback-backed files are reset.',
  );
  ...
  console.log('Example:');
  console.log(
    '  node scripts/extract-game-data-from-official.js --character-id 12345 --hash AbCdEf123 --cookie-file tmp/api.cookie.txt --inspect-output tmp/initClientData.decoded --save-raw tmp/initClientData.raw.json',
  );
}
...
function extractClientDataFromMessage(payload) {
  ...
  const candidates = [payload?.data, payload?.initClientData, payload?.initClientDataData, payload?.clientData];
...
async function main() {
  ...
  if (cookie) {
    ...
    if (authStatus !== 200) {
      throw new Error(
        `Cookie auth check failed (/v1/characters -> ${authStatus}). Please refresh cookie from browser.`,
      );
    }
  } else {
  ...
  if (reset.length > 0) {
    console.log(
      `Reset ${reset.length} optional tracked game-data file to fallback because the payload did not include it:`,
    );
  ...
  if (inspectOutputDir && inspectOutputDir !== outputDir) {
    ...
    if (inspectReset.length > 0) {
      console.log(
        `Also reset ${inspectReset.length} optional tracked game-data file for inspection because the payload did not include it:`,
      );
    ...
    if (inspectSkipped.length > 0) {
      console.log(
        `Also skipped ${inspectSkipped.length} tracked game-data file for inspection because the payload did not include it:`,
      );
